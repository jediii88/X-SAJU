...
function yearlyFourDomainKeywords(score, sewSip) {
    ...
    var w, cr, doc, love;
    if (score >= 3) {
        w = '알트·테마주 단타·온체인 변동성 구간 — 리스크 리워드 공격';
        cr = 'AI·데이터·플랫폼 축 이직·승진 신호, 퍼스널 브랜딩 확장';
        doc = /식신|상관|편재|정재/.test(sip) ? '스마트계약·저작권·SAAS 계약 체결 유리' : '노션·슬랙·메일 증빙 파이프라인만으로도 실속';
        love = /식신|편재|정재|비견/.test(sip) ? '데이팅 앱·인스타 DM·오픈채 귀인 확률↑' : '알고리즘 추천 모임에서 신뢰 스택 쌓기';
    } else if (score >= 1) {
        w = '분할매수·배당ETF·현금흐름 우선, 코인 비중은 소액만';
        cr = '프롬프트·자동화 스킬업, 이력 깃허브·포폴 정비';
        doc = /식신|상관|편인/.test(sip) ? 'PRD·API명세 선행 시 승인 빨라짐' : '전자계약 조항·감사로그 재확인 후 서명';
        love = /정재|편관|정관/.test(sip) ? '유료 커뮤니티·소개팅 앱 필터 엄격히' : '영상통화 1회 후 오프라인 — 캣피싱 차단';
    } else if (score === 0) {
        w = '레버리지 금지, 현금·ETF 코어 유지';
        cr = '현 포지션 유지·업스킬 1코스만';
        doc = '클라우드 증빙·세무 스캔 루틴 점검';
        love = 'DM·단톡 과몰입 줄이고 경청 우선';
    } else if (score >= -2) {
        w = '코인·테마주 비중 10% 미만, 방어형 ETF로 재편';
        cr = '노출·정치 슬랙 최소화, 리스크 레지스터';
        doc = '독소 SaaS조항·자동연장 해지일 점검';
        love = '새 매칭·오픈채 대량추가 중단';
    } else {
        w = '청산·스탑로스·생존 현금흐름';
        cr = '조직 갈등 거리두기, 컴플라이언스 선제';
        doc = '법무·세무 원스톱 자문 확보';
        love = '차단·뮤트 후 관계 정돈';
    }
...
function yearlyFourDomainIndicators(score, sewSip) {
    ...
    if (score >= 3) {
        w = '알트·단타공세';
        cr = 'AI직무승부';
        doc = /식신|상관/.test(sip) ? '온체인계약' : '클라우드증빙';
        love = '앱·DM귀인';
    } else if (score >= 1) {
        w = 'ETF·분할매수';
        cr = '프롬프트정비';
...
function yearlyDomainStrategicAdvices(score) {
    ...
    return {
        wealth: sc >= 2
            ? '길운이 밀어줄 때 변동성 큰 **알트코인·테마주 단타**나 레버리지 파생에서 한탕이 나올 수 있습니다. 다만 **손실 한도·청산가**를 앱에 먼저 박으십시오. 미수·스테이킹·에어드랍은 **한 주에 한 파이프라인**만 열고, 입금·출금 문자는 스크린샷으로만 맞추십시오.'
            : sc >= 0
            ? '벌어도 손에 잘 안 남는다면 고정비·구독·코인 평가손이 조용히 새는 겁니다. **미국 대형 기술주·배당주 ETF**로 코어를 잡고, 가상자산은 **10% 미만**으로 묶으십시오. 카드·자동이체는 주 단위로만 열어보십시오.'
            : '누군가에게 끌려 지갑이 열렸을지 모릅니다. 당신 잘못이 아니라 경계 학습이 늦었을 뿐입니다. **연대보증·지분·가족 명의 대출·코인 P2P**는 올해 서명하지 마십시오. 꼭 필요하면 변호사·세무 검토 전 두 번째 페이지는 넘기지 마십시오.',
        career: sc >= 2
            ? '표면 승진보다 **생성형 AI·자동화 파이프라인**을 손에 쥔 쪽이 이깁니다. **프롬프트·RAG·에이전트 워크플로** 한 축을 포트폴리오로 고정하고, 연봉 협상에는 **직무 범위·보고 라인·야근 유무**를 이메일로 먼저 받아 두십시오.'
            : sc >= 0
            ? '조용히 해도 일이 늘기만 했다면 경계가 흐린 자리에 선 것입니다. **회의록마다 R&R 한 줄**과 **링크드인·깃허브에 분기 한 건**만 박으십시오. 상사 교체·프로젝트 합병 때는 성과 지표를 다시 쓰게 한 뒤에만 새 업무에 손대십시오.'
            : '속으로는 이미 여러 번 사직서를 썼을지 모릅니다. 그래도 당신은 버틴 것입니다. **이력서는 비공개로만** 올리고, 병가·연차로 호흡을 확보한 뒤 **데이터·제품·매출** 지표 한 줄을 문서로 확보한 뒤 내부 전배로만 협상하십시오.',
        love: sc >= 2
            ? '알고리즘 기반 **데이팅 앱(틴더·범블·글램 등)**이나 **인스타그램 DM**으로 비대면 접점이 열리면, 의외의 귀인이 붙기 쉽습니다. 자만추만 고집하지 마십시오. 첫 만남에서 **돈·코인·지분** 이야기가 나오면 그날은 관계만으로 끊으십시오.'
            : sc >= 0
            ? '연락이 잦을수록 오해가 쌓입니다. **오픈채팅·디스코드**는 취향 필터가 맞을 때만, 짧은 음성·영상통화 1회 후 오프라인으로 넘기십시오. **주 2회 고정 시간** 외 푸시는 끄십시오.'
            : '말이 통하지 않았다고 탓하지 마십시오. **헤어진 이·단톡·DM 속 지인**에게 지갑·코인·투자 이야기를 하지 마십시오. “지금은 답 못 준다” 한 문장만 반복하십시오.',
        health: sc >= 2
...
function buildChapter4_Wealth(data) {
    ...
    const wealthDesc = wealthType === 'active'
        ? `${name}님은 현금이 손을 타고 흐를 때 힘이 납니다. 월급만으로는 속도가 안 맞을 수 있습니다. 그건 욕심이 아니라 구조입니다. **테마주·알트·부업 파이프라인은 세 개 넘기지 마십시오.** 길운 월에만 스캘핑·공모주 승부를 허용하십시오.`
        : `${name}님은 ‘이름·전문성·신뢰’가 쌓일수록 돈이 따라붙는 축입니다. 한탕보다 **배당 ETF·대형 기술주 코어**와 반복 계약이 맞습니다. **브랜딩 한 줄·현금흐름 표**를 먼저 고정하십시오.`;

    const wealthCaution = wealthType === 'active'
        ? `속도가 곧 함정입니다. 코인·옵션 **과레버 한 번**이 전부를 가져갑니다. **스탑로스·보증 거절**을 문장으로 적어 지갑 옆에 두십시오.`
        : `돈을 쫓으면 도망갑니다. 가격표를 먼저 올리십시오. **가상자산 비중 10% 미만**을 지키고, 분기마다 **증명 가능한 성과** 한 가지만 대외에 보내십시오.`;
    ...
    return `<div class="report-chapter">
        ...
        <p class="ch-text">재물은 감이 아니라 리듬입니다. 벌 때와 줄 때를 구분하지 못하면 같은 실수가 반복됩니다. **용신 구간은 변동성 자산·테마주·온체인까지 공격**, 기신 구간은 **ETF·현금 코어**로만 수비하십시오.</p>
...
function buildDaewunLoop(data) {
    ...
    rows.forEach((r, idx) => {
        ...
        const WEALTH_JI = {
            ...
            '寅':'움직임이 돈입니다. **이사·이직·테마주·새 온라인 채널** 중 하나만 고르십시오.',
            '卯':'사람이 문을 엽니다. **소개·협업 제안**을 먼저 보내십시오.',
            '辰':'변수가 겹칩니다. **신규 코인·레버리지는 유예**하고 관망하십시오.',
            '巳':'서명이 필요합니다. **조건을 먼저 글로** 적고 도장을 찍으십시오.',
            '午':'평판이 수입입니다. **약속 이행**을 먼저 쌓으십시오.',
            '未':'감성이 상품이 됩니다. **콘텐츠·교육** 하나를 완성하십시오.',
            '申':'속도전입니다. **스캘핑·옵션·알트**는 손실 한도 숫자를 적고 움직이십시오.',
        ...
        // 애정 조언
        const loveAdv = sc>=2 ? '**데이팅 앱·인스타 DM**에서 접점이 열립니다. **기준 한 줄**을 프로필에 먼저 쓰십시오.' : sc>=0 ? '안정 국면입니다. **영상통화 1회 후 오프라인**·대화 시간 고정이 관계를 지킵니다.' : '오해 비용이 큽니다. **DM·단톡은 스크린샷**으로 남기고 감정 결정은 유예하십시오.';
...
function buildSewunLoop(data) {
    ...
    const GAN_DESC = {
        ...
        '壬':'흐름과 확산. ETF·온체인·글로벌 채널로 자본이 퍼지는 국면.',
...
function buildChapter6_Love(data) {
    ...
    return `<div class="report-chapter">
        ...
        ${buildDomainSummaryTable({
            boxTitle:'애정운 핵심 요약',
            keyword:'알고리즘 매칭 · DM 리듬 · 리스크 리워드',
            route:'데이팅 앱 필터·오픈채 취향 정렬 후 영상통화 1회 고정',
            caution:'감정 최고점·코인·지분 이야기가 겹친 날의 관계 결정은 유예하십시오'
        })}
...
function buildZiWeiDestinyBlueprintSection(data) {
    ...
    var Z = [
        ...
        { k: '파군', h: '破军', p: '부수고 다시 짓는 루프에 익숙한 혁신가입니다.', f1: '파괴적 리밸런싱(올인·청산)을 피하려면 **ETF·나스닥 코어 70%**를 먼저 잠그십시오.', f2: '관록은 **신사업·POC**에서 이름이 오릅니다.', o: '천이에서 **해외·원격** 제안이 들어오면, 계약 관할을 먼저 쓰십시오.', a: '“이번 달 손볼 일 한 가지”만 메모 상단에 두십시오.' }
    ];
    var M = Z[seed];
    var C = Z[(seed + 4) % 14];
    var G = Z[(seed + 7) % 14];
...
const FORTUNE_TEXT_DB = {
    sipseong: {
        '비견': [
            ...
            '지분·스테이킹·공동지출에서 **한 줄 합의**가 승패를 가릅니다.'
        ],
        '겁재': [
            '자원 쟁탈전이 본격화됩니다.',
        ...
        '편재': [
            ...
            '외부 자본과 거래 스케일이 커집니다.',
            '빠른 회전의 수익 기회가 열립니다.',
            '알트·테마주·단기 파생에서 **리스크 리워드**가 커집니다.'
        ],
        '정재': [
            '현금흐름과 고정수익을 다지는 구간입니다.',
            '월 단위 수익 구조를 재정렬해야 합니다.',
            '지출 통제와 누적 자산 축적이 핵심입니다.',
            '배당주·ETF·월세 파이프라인으로 **파이낸셜 프리덤** 코어를 잡으십시오.'
        ],
        '편관': [
    ...
    action: {
        favorable: [
            ...
            '길운이 몰리는 달에는 **공모주·테마주 스캘핑** 한 파이프라인만 열고 나머지는 잠그십시오.'
        ],
        cautious: [
            ...
            '코인·알트 **충동 매수**는 이번 달 금지 — **ETF 적립**만 허용하십시오.'
        ]
    }
};
