...
function generateDeepReport(data) {
    ...
    // ── 1부: 나라는 사람 ──
    var part1Body = '';
    //   ① 일주 카드 — 만세력 안내 → 원국 표 → 원국 해부(인생 큰 흐름·직업·관계 보강)
    //   ② 오행 · 십성 — 참고 분석
    //   ③ 한 줄기 서사 — 태어남부터 이어지는 통합 서사
...
function buildChapter3_Sipseong(data) {
    ...
    const groupRank = SIP_BAR_GROUPS.map(function (g) {
        var sum = g.keys.reduce(function (s, k) { return s + (Number(sipseong[k]) || 0); }, 0);
        return { key: g.key, label: g.label, sum: sum, pct: Math.round(sum / total * 100) };
    }).sort(function (a, b) { return b.sum - a.sum; });
    const topG = groupRank[0] || { key: '비겁', label: '비겁 (비견/겁재):', pct: 0 };
...
function buildTOC(data) {
    ...
    var groups = [
        ...
        { head: '1부 · 나라는 사람', sub: '원국에서 비친 줄기', items: [
            ...
            { t: '원국 해부 — 인생 큰 흐름', s: '표 직후 종합 풀이 · 직업·관계 보강', p: '—' },
            { t: '오행 · 십성', s: '참고 분석', p: '—' },
