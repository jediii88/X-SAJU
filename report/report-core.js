...
function injectSectionInterpretations(data) {
    ...
    const yongDetail = {
        ...
        metal: '금 기운을 메인으로 삼으면 결단·전문성이 살아납니다. 가을(8~10월), 경·신년·경·신 대운이 전성기 축입니다. 서쪽·흰색·IT·법·의료가 길합니다. **돈·기간·분쟁이 크게 묶이는 계약(전세·근로·투자·가맹·연대보증 등)**은 감정이 오른 날 말고, 받은 날 곧바로 전자서명·날인하지 말고 **영업일 이틀만 비운 뒤** 같은 맥락으로 처리하십시오. 그 사이에는 조건·해지만 메모로 적어두면 됩니다. **소액 쇼핑·멤버십·일반 회원가입**은 여기 해당 없습니다.',
...
function pickSeYunYearHookParagraph(name, yr, gHan, jHan, evLabel, sc) {
    ...
    var hooks = [
        ...
        function () { return yk + '에는 **파이프라인이 갈라지는 달**이 분명히 있습니다. ' + nmEunNeun(nm) + ' 그 달의 월요일 아침에만 “메인 한 가지”를 적고, 나머지 제안은 다음 주로 넘기십시오.'; },
...
function buildChapter1_Basic(data) {
    ...
    let lifeNarr = (function() {
        ...
        var strongText = isStrong
            ? nmUi(name)+' 사주는 신강(身强)에 가깝습니다. 에너지가 넘치면 방향만 틀려도 큰 마찰이 생깁니다. 그건 성격이 나빠서가 아니라, 일간의 기운이 원국에서 먼저 서 있기 때문입니다. **옳은 방향을 잡으면 폭발적인 성과가 나옵니다.** 소진이 빠르므로, 쉬는 날을 죄책감 없이 넣으십시오.'
            : nmUi(name)+' 사주는 신약(身弱)에 가깝습니다. 혼자 끌어가려다 지친 적이 있었을 수 있습니다. 그건 나약함이 아니라, 기운이 사람·환경과 맞물릴 때 배가 되는 구조입니다. **용신 쪽 기운이 있는 자리를 가까이 두면 체감이 달라집니다.**';
...
${nmEunNeun(name)} ${seasonKr}에 태어났습니다. 이 계절의 기운이 사주 전체의 온도와 습도를 결정합니다. ${johu.split('.').slice(-3).join('.').trim()} 이 균형이 맞는 대운·세운이 찾아올 때 삶이 가장 크게 도약합니다.`;
    ...
    return `<div class="report-chapter chapter-start">
        <h3 class="ch-title">[ 01. 원국 해부 ] — 태어난 순간의 별자리 지도 — ${nmUi(name)} 인생 흐름</h3>
