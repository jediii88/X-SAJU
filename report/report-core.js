...
function yearlyFourDomainKeywords(score, sewSip) {
    ...
    if (score >= 3) {
        ...
        cr = '데이터·자동화·대외 브랜딩 축 이직·승진 신호';
        doc = /식신|상관|편재|정재/.test(sip) ? '전자계약·저작권·구독형 서비스 계약 체결에 유리' : '메일·문서 증빙 파이프라인만으로도 실속';
        love = /식신|편재|정재|비견/.test(sip) ? '매칭·메시지 채널에서 접점 확률↑' : '추천 모임에서 신뢰를 쌓기';
    } else if (score >= 1) {
        w = '분할 매입·배당·현금흐름 우선, 고변동 자산은 소액만';
        cr = '업무 자동화 역량·이력·실적 정리';
        doc = /식신|상관|편인/.test(sip) ? '요구사항·명세 선행 시 승인이 빨라짐' : '전자계약 조항·감사로그 확인 후 서명';
        love = /정재|편관|정관/.test(sip) ? '직무 커뮤니티·소개 경로 필터를 엄격히' : '영상 미팅 1회 후 대면 — 신원·조건 확인';
    } else if (score === 0) {
        w = '레버리지 금지, 현금·우량 코어 유지';
        cr = '현 포지션 유지·업스킬 1코스만';
        doc = '클라우드 증빙·세무 스캔 루틴 점검';
        love = '메시지·단체 채널 과몰입 줄이고 경청 우선';
    } else if (score >= -2) {
...
function sipToIndustryWorkStyle(sip) {
    var m = {
        ...
        '식신': '**IT·콘텐츠·디자인·기획**처럼 아이디어가 바로 산출물로 이어지는 일',
...
function yearlyFourDomainIndicators(score, sewSip) {
    ...
    if (score >= 3) {
        w = '단기·고변동 공세';
        cr = '데이터·자동화 직무 경쟁';
        doc = /식신|상관/.test(sip) ? '전자계약' : '클라우드 증빙';
        love = '매칭·메시지 귀인';
    } else if (score >= 1) {
        w = '분할·배당 우선';
        cr = '업무 설계 정비';
...
function yearlyDomainStrategicAdvices(score) {
    ...
    return {
        wealth: sc >= 2
            ? '길운이 밀어줄 때 **고변동·단기 매매**나 레버리지 성격의 상품에서 한탕이 나올 수 있습니다. 다만 **손실 한도·청산 기준**을 숫자로 먼저 박으십시오. 미수·고수익 이벤트성 투자는 **한 주에 한 파이프라인**만 열고, 입금·출금 내역은 증빙으로만 맞추십시오.'
            : sc >= 0
            ? '벌어도 손에 잘 안 남는다면 고정비·구독·평가손이 조용히 새는 겁니다. **대형 우량·배당형 펀드**로 코어를 잡고, **고변동 자산은 10% 미만**으로 묶으십시오. 카드·자동이체는 주 단위로만 열어보십시오.'
            : '누군가에게 끌려 지갑이 열렸을지 모릅니다. 당신 잘못이 아니라 경계 학습이 늦었을 뿐입니다. **연대보증·지분·가족 명의 대출·비공식 투자 권유**는 올해 서명하지 마십시오. 꼭 필요하면 법무·세무 검토 전 두 번째 페이지는 넘기지 마십시오.',
        career: sc >= 2
            ? '표면 승진보다 **업무 자동화·디지털 파이프라인**을 손에 쥔 쪽이 이깁니다. **요건 정리·자동화 워크플로** 한 축을 포트폴리오로 고정하고, 연봉 협상에는 **직무 범위·보고 라인·야근 유무**를 공식 메일로 먼저 받아 두십시오.'
            : sc >= 0
            ? '조용히 해도 일이 늘기만 했다면 경계가 흐린 자리에 선 것입니다. **회의록마다 역할 한 줄**과 **이력·실적 요약에 분기 한 건**만 박으십시오. 상사 교체·프로젝트 합병 때는 성과 지표를 다시 쓰게 한 뒤에만 새 업무에 손대십시오.'
            : '속으로는 이미 여러 번 사직서를 썼을지 모릅니다. 그래도 당신은 버틴 것입니다. **이력서는 비공개로만** 올리고, 병가·연차로 호흡을 확보한 뒤 **데이터·제품·매출** 지표 한 줄을 문서로 확보한 뒤 내부 전배로만 협상하십시오.',
        love: sc >= 2
...
function buildYearStrategicNarrative(name, yr, kor, evLabel, sc, sewSip, ohTag, domainFour) {
    ...
    var soul = SEYUN_SIP_SOUL_METAPHOR[sip] || (yr + '년에는 ' + sipToIndustryWorkStyle(sip).replace(/\*\*/g, '') + ' 리듬이 겹쳐, 같은 일정도 체감 속도가 달라집니다.');
    ...
    if (isXing && sc >= 2) {
        var a = [
            nm + '님은 기회를 놓칠까 봐 잠을 줄였을 수 있습니다. 겉으로는 당당해 보였을지라도, 속으로는 늘 시계를 보고 계셨을 것입니다.',
            nm + '님께는 “지금 아니면 안 된다”는 말이 귀에 박혔을 수 있습니다. 그 말에 끌려가며, 정작 쉬는 법은 잊어버리셨을지 모릅니다.',
            nm + '님은 주변을 챙기느라 정작 본인의 숫자는 뒤로 미뤘을 수 있습니다. 그런데도 당신이 이기적인 사람이었던 것은 아닙니다.',
            nm + '님께는 인정받고 싶은 마음이 커서, 손해 보는 자리에도 앉아 있었을 수 있습니다. 그건 약함이 아니라, 믿음이 컸기 때문입니다.'
        ];
        step1 = a[mix];
    } else if (isXing) {
        var b = [
            nm + '님은 분명히 앞서가려 했을 것입니다. 그런데도 속으로는 “이만하면 됐나” 하는 불안이 끊기지 않았을 수 있습니다.',
            nm + '님께 좋은 소식이 왔을 때조차, 먼저 오는 건 걱정이었을지 모릅니다. 그 마음이 이상한 게 아닙니다.',
            nm + '님은 웃으며 받아들였을지 모릅니다. 그러나 밤에는 혼자서 숫자를 다시 셌을 수 있습니다.',
            nm + '님께는 “한 번만 더”가 쌓여, 어느새 어깨가 굳었을 것입니다. 그건 당신이 못나서가 아닙니다.'
        ];
        step1 = b[mix];
    } else if (isZhuyi) {
        var c = [
            nm + '님은 말 한마디에 책임이 몰려, 억울한 밤을 보냈을 수 있습니다. 당신을 오해한 사람들이 있었을 것입니다. 그러나 당신은 틀리지 않았습니다.',
            nm + '님께는 “왜 나만”이 목구멍까지 올라왔을 수 있습니다. 그 감정을 숨기느라 더 지치셨을지 모릅니다.',
            nm + '님은 조용히 버텼을 수 있습니다. 그런데도 마음속으로는 이미 여러 번 무너졌을 수 있습니다.',
            nm + '님께는 미안함이 먼저 올라와, 본인의 몫을 줄였을 수 있습니다. 그건 착해서입니다, 무능해서가 아닙니다.'
        ];
        step1 = c[mix];
    } else {
        var d = [
            nm + '님께는 큰 드라마보다, 잔잔한 불안이 더 오래 남았을 수 있습니다. 겉으로는 괜찮다고 했을지라도, 속으로는 늘 점검 목록이 돌았을 것입니다.',
            nm + '님은 “별일 없다”는 말을 믿고 싶었을 수 있습니다. 그런데도 몸은 먼저 긴장하고 있었을지 모릅니다.',
            nm + '님께는 변화가 조용히 와서, 나중에야 알아챘을 수 있습니다. 늦게 깨달았다고 해서 잘못된 것은 아닙니다.',
            nm + '님은 사람 좋다는 말을 많이 들었을 수 있습니다. 그 말이 때로는 족쇄처럼 느껴졌을지 모릅니다.'
        ];
        step1 = d[mix];
    }

    var step2Tails = [
        '<strong>' + nm + '님께는 이때는 일정 겹침을 줄이려면, **요일별로 “메인 한 줄”만** 남기고 나머지는 다음 주로 미루십시오.</strong>',
        ...
        '<strong>' + nm + '님께는 **겹치는 약속을 두 줄로 쪼개** 한 줄은 이메일로만 확정하십시오.</strong>',
    ...
    var integr = [
        nm + '님은 ' + yr + '년 상반기에 끝낼 일 두 가지만 적으십시오. 끝나기 전에는 새 동업 미팅에 가지 마십시오.',
        nm + '님은 ' + yr + '년에 고정비·저축·외상 한도 숫자 한 장만 맞추면 싸움이 줄어듭니다. 숫자 맞춘 날에는 술자리를 피하십시오.',
        nm + '님은 ' + yr + '년 외부에 드러낼 축을 하나만 정하십시오. 나머지는 조용히 지키는 편이 돈이 덜 샙니다.',
        nm + '님은 ' + yr + '년 말에 서명 문서 스크린샷 한 폴더만 만들어도 다음 해가 편해집니다.'
    ][(mix + yr) % 4];
...
function injectSectionInterpretations(data) {
    ...
    const yongDetail = {
        ...
        metal: '금 기운을 메인으로 삼으면 결단·전문성이 살아납니다. 가을(8~10월), 경·신년·경·신 대운이 전성기 축입니다. 서쪽·흰색·IT·법·의료가 길합니다. **서명은 영업일 이틀 유예**하십시오.',
...
💰 재물 — 신규 수입·투자·확장은 <strong>예산·회수일·중도해지</strong> 세 줄을 종이에 적은 뒤에만 착수하십시오.

💼 직업 — 역할이 커질수록 <strong>보고 라인과 R&R</strong>를 이메일로 고정하십시오. 속도보다 방향이 맞을수록 상승 구간이 깁니다.
...
💰 재물 — 투기보다 <strong>현금흐름·복리·지출 구조</strong> 세 가지를 주 단위로만 열어보십시오.
...
💰 재물 — 큰 투자·보증·고정비 확대는 보수적으로 접근하십시오. <strong>비상자금 비율</strong>을 먼저 숫자로 적으십시오.
...
💰 올해 재물운 — 신규 수입·투자·확장은 <strong>기준·예산·회수일</strong>을 먼저 적은 뒤에만 움직이십시오. 번 돈을 지키는 것이 곧 재물 전략입니다.
...
💰 올해 재물운 — 투기보다 <strong>복리·지출 구조·현금흐름</strong>에만 집중하십시오. 소액이라도 같은 요일에 저축이 들어가게 고정하십시오.
...
💰 올해 재물운 — 신규 투자·보증·큰 계약은 <strong>영업일 이틀을 비운 뒤</strong> 서명하십시오. 비상자금 비율을 숫자로 먼저 적으십시오.
...
🏥 올해 건강운 — 쌓인 피로가 나중에 터질 수 있습니다. <strong>수면 7시간·음주 주 2회 이하·밤 11시 이후 근무 중단</strong>을 마지노선으로 삼으십시오.`;
    ...
    return `<div class="report-chapter" style="border:2px solid var(--gold);border-radius:14px;padding:20px;background:rgba(199,167,106,0.04);">
        ...
        <h3 class="ch-title" style="margin-top:0;">현재 대운·세운·월운 종합 분석 — 지금 당신의 운은 어디로 흐르는가</h3>
...
function buildMasterFullReading(data) {
    ...
    return `<div class="report-chapter">
        <h3 class="ch-title">📖 원국 한번에 짚기 — 성격·돈·일·사랑·몸</h3>
...
function pickSeYunYearHookParagraph(name, yr, gHan, jHan, evLabel, sc) {
    ...
    var hooks = [
        function () { return yk + '의 흐름은 **계약·송금·사람 약속**이 같은 주에 겹치기 쉬운 타입입니다. ' + nm + '님은 그 주 **첫 업무 블록 하나**만 메인 일정으로 먼저 잡으십시오.'; },
        ...
        function () { return nm + '님께 ' + yk + '는 **대외 일정**이 먼저 밀려오고 뒤늦게 내부 정리가 따라붙는 패턴입니다. 겉 일정표와 속 장부를 **월요일·목요일 두 번만** 맞추십시오.'; },
...
function buildPremiumExecutiveSummary(data) {
    ...
    var coreFusion = nm + '님께는 환경이 바뀔수록 숫자와 사람 약속이 한꺼번에 몰리는 구조가 있습니다. 그건 성격 문제가 아니라, **추진력과 정밀함이 동시에 요구되는 커리어 패턴**과 별자리 기반 「' + palaceLabel + '」 무대가 겹친 탓입니다. <strong>흔들림을 기다리지 말고 변화의 순서를 직접 설계하십시오.</strong>';

    var pools = [
        {
            m1: nm + '님은 ' + iljuImage + '의 구조를 가진 고출력 엔진과도 같습니다. 연료가 넘칠수록 과열이 걱정되었을 수 있습니다. 【확장은 멈추고 ' + yongKr + ' 냉각만 가동】하십시오. ' + giKr + ' 방향의 충동 확장은 즉시 끊으십시오.',
...
function buildChapter1_Basic(data) {
    ...
    let lifeNarr = (function() {
        ...
        var strongText = isStrong
            ? name+'님의 사주는 신강(身强)에 가깝습니다. 에너지가 넘치면 방향만 틀려도 큰 마찰이 생깁니다. 그건 성격이 나빠서가 아니라, 일간의 기운이 원국에서 먼저 서 있기 때문입니다. **옳은 방향을 잡으면 폭발적인 성과가 나옵니다.** 소진이 빠르므로, 쉬는 날을 죄책감 없이 넣으십시오.'
            : name+'님의 사주는 신약(身弱)에 가깝습니다. 혼자 끌어가려다 지친 적이 있었을 수 있습니다. 그건 나약함이 아니라, 기운이 사람·환경과 맞물릴 때 배가 되는 구조입니다. **용신 쪽 기운이 있는 자리를 가까이 두면 체감이 달라집니다.**';
        ...
        var GG_LIFE = {
            ...
            '편재격':'사람과 기회 사이에서 흐름을 만드는 구조입니다. 한 축보다 여러 파이프라인이 맞습니다. **새 통장·새 앱은 분기에 하나만** 허용하십시오.',
        ...
        if(gmHitPillars.length > 0) {
            ...
            gmText = '\n\n⚠ '+name+'님의 사주에는 '+gmKr+' 기운이 공망입니다. 갈망이 큰 영역일수록 결정을 서두르고 싶어집니다. 그건 결핍이 아니라, 그 기운을 더 깊이 파고들라는 신호에 가깝습니다. **공망이 강하게 들어오는 대운·세운에는 계약 서명을 이틀 유예**하십시오.';
        }
        ...
        if(iljuCore || iljuImgData) {
            ...
            narr += name+'님의 사주는 그림으로 표현하면 — 「'+imgTxt+'」입니다.\n';
            if(descTxt) narr += descTxt + '\n';
            if(iljuCore) narr += iljuCore.replace(/당신/g, name+'님') + ' **일주의 무게를 탓하지 마십시오. 한 달에 바깥에 꺼낼 목표는 하나만 고르십시오.**\n\n';
        }
        ...
        // 3) 나이대 흐름
        narr += '\n━━━━━━━━━━━━━━━━\n';
        narr += '🌱 유년기(0~15세) — '+name+'님의 뿌리\n';
        narr += '━━━━━━━━━━━━━━━━\n';
        var YEAR_LIFE_SIMPLE = {
            earth: name+'님의 0~15세는 안정을 중시하는 가정 환경 속에서 원칙과 책임감이 형성되는 시기입니다. 이 시기에 익힌 성실함과 인내가 평생의 중심축이 됩니다. ▸ 흥망: 이 시기를 어떻게 보냈느냐에 따라 평생의 안정 기반이 달라집니다. ▸ 보완: 너무 이른 책임감이 짐이 되지 않도록, 실패해도 된다는 경험을 쌓는 것이 중요합니다.',
            wood: name+'님의 0~15세는 개척적이고 주도적인 기질이 발현되는 시기입니다. 또래보다 먼저 무언가를 시작하려는 본능이 두드러집니다. ▸ 흥망: 이 에너지가 긍정적으로 발현되면 리더십으로, 억압되면 반항심으로 나타납니다. ▸ 보완: 새로운 것을 배울 기회를 충분히 주면 타고난 잠재력이 꽃을 피웁니다.',
            fire: name+'님의 0~15세는 표현력과 존재감이 남다른 시기입니다. 어릴 때부터 사람들 앞에 서는 것을 좋아하고 주목받는 경험이 자신감의 기초가 됩니다. ▸ 흥망: 표현과 인정의 경험이 풍부할수록 성인 이후 리더십이 강해집니다. ▸ 보완: 주목받지 못하면 크게 흔들릴 수 있으니, 작은 성취도 충분히 인정해주는 환경이 중요합니다.',
            metal: name+'님의 0~15세는 기준이 높고 완벽을 추구하는 기질이 형성되는 시기입니다. "왜?"라는 질문과 높은 완성도 의식이 어릴 때부터 나타납니다. ▸ 흥망: 이 기질이 잘 발달하면 탁월한 전문가로, 억압되면 완벽주의 함정에 빠집니다. ▸ 보완: 80점도 충분히 잘했다는 경험이 성인 이후 실행력을 높입니다.',
            water: name+'님의 0~15세는 섬세한 관찰력과 사색적 감수성이 형성되는 시기입니다. 남들보다 더 많이 느끼고 생각하는 내면의 풍부함이 이 시기에 뿌리를 내립니다. ▸ 흥망: 이 감수성이 잘 발달하면 깊은 통찰력으로, 너무 억압되면 사회 불안으로 발전할 수 있습니다. ▸ 보완: 감정을 말·일기·운동 등 건전한 통로로 바깥으로 보내는 습관을 어릴 때부터 익히는 것이 중요합니다.'
        };
        ...
        narr += '━━━━━━━━━━━━━━━━\n';
        var MID = {
            fire: name+'님의 35~55세는 【흥(興)의 절정기】입니다. 직업: 20~30대에 흩어졌던 에너지가 하나로 모이며, 처음으로 "내 힘으로 만든 결과"가 세상에 드러납니다. 이 시기에 사업 확장·직급 상승·사회적 인정이 집중됩니다. 재물: 가장 많이 벌고 가장 많이 쓰는 시기입니다. 수입이 늘어나는 만큼 장기 자산 구조를 잡아야 합니다. 보완: 과로와 건강 방치가 이 시기 최대 위험입니다. 연 1회 이상 정기 검진을 가능하면 받으십시오.',
            wood: name+'님의 35~55세는 방향이 잡히며 진짜 도약이 시작되는 시기입니다. 직업: 35세 전후 중요한 방향 전환 결정이 기다립니다. 이 결정이 이후 20년을 결정합니다. 늦었다고 느껴도 지금이 가장 빠른 시작입니다. 재물: 초기 투자·창업·신규 수입원 개척에 가장 강한 에너지를 받는 시기입니다. 보완: 너무 많은 방향으로 동시에 뻗으려는 충동을 제어하십시오. 하나에 집중해야 결과가 납니다.',
            earth: name+'님의 35~55세가 인생 전성기입니다. 직업: 지금까지 묵묵히 쌓아온 신뢰와 전문성이 가장 강력한 무기가 됩니다. 이 시기에 안정적 직위·사업 기반·사회적 신망이 확보됩니다. 재물: 부동산·장기 투자·안정적 자산 구축에 최적의 시기입니다. 보완: 너무 안정만 추구하다 기회를 놓치지 않도록, 계산된 리스크를 감수하는 용기가 필요합니다.',
            metal: name+'님의 35~55세는 전문성이 권위로 전환되는 시기입니다. 직업: 이 시기에 해당 분야에서 타의 추종을 불허하는 전문성이 완성됩니다. 누군가 의견을 구하는 사람이 되는 것이 목표입니다. 재물: 전문성 기반의 고수익 구조를 만드는 시기입니다. 컨설팅·강의·지식 판매 등 전문성을 상품화하십시오. 보완: 고집이 강해지는 시기입니다. 새로운 방식에 열린 태도를 의식적으로 유지하십시오.',
            water: name+'님의 35~55세는 오랜 축적이 가치로 전환되는 시기입니다. 직업: 분석력과 통찰이 인정받기 시작합니다. 자연스럽게 조언을 구하는 사람들이 모여드는 시기입니다. 재물: 정보와 지식이 돈으로 전환됩니다. 자신의 노하우를 체계화하면 장기적 수입원이 됩니다. 보완: 불안감이 커지기 쉬운 시기입니다. 행동하지 않으면 기회가 지나갑니다. 계획이 충분할 때에는 단계적으로 실행에 옮기십시오.'
        };
        narr += (MID[dOh]||name+'님의 35~55세는 인생의 꽃이 피는 시기입니다.') + '\n';
...
${name}님은 ${seasonKr}에 태어났습니다. 이 계절의 기운이 사주 전체의 온도와 습도를 결정합니다. ${johu.split('.').slice(-3).join('.').trim()} 이 균형이 맞는 대운·세운이 찾아올 때 삶이 가장 크게 도약합니다.`;
        }
        ...
        if(gg && gg.geokName && gg.info) {
            ...
            const GG_NARR = {
                '정재격': isSeong
                    ? `재물에 관해 꼭 기억하셔야 할 것이 있습니다. ${name}님이 돈을 버는 가장 강력한 방식은 화려한 한탕이 아닙니다. 한 사람 한 사람에게 쌓아온 신뢰가 결국 자산이 됩니다. 빠른 돈벌이를 좇아 이직을 반복하거나 충동적 투자에 뛰어드는 시기가 오면, 그때가 오히려 가장 위험한 순간입니다. 지금 있는 자리에서 신뢰를 쌓으십시오. 그것이 10년 후 가장 큰 자산이 됩니다.`
                    : `${name}님, 재물과 관련해서 반복되는 위험한 패턴이 있습니다. 가까운 사람에 의해 돈이 새거나 막히는 상황이 생깁니다. 이것을 막는 방법은 하나입니다 — 돈이 오고 가는 모든 관계에 계약서와 문서를 남기십시오. 감정이 있는 관계일수록 더 철저히 해야 합니다.`,
                '편재격': isSeong
                    ? `${name}님에게 고정 월급만으로 사는 삶은 답답하실 수 있습니다. 검증된 부업·프로젝트 채널을 단계적으로 여시면 숨통이 트입니다. 사람과 사람 사이에서 기회를 만들어내고, 아이디어가 돈이 되는 방식이 이 사주에 맞는 구조입니다. 큰 판을 벌이는 것을 겁내지 않는 편이 좋습니다. 단, 한 번에 모든 것을 거는 도박은 피하십시오. 여러 판을 동시에 벌이는 것이 전략입니다.`
                    : `${name}님은 재물을 크게 다루는 감각이 있지만, 한 번의 판단 실수로 크게 잃는 리스크가 함께 옵니다. 큰 투자 전에 가능하면 안전장치를 먼저 만드십시오. 잃어도 다시 시작할 수 있는 구조를 유지하는 것이 최우선입니다.`,
                '정관격': isSeong
                    ? `${name}님이 사회에서 인정받는 방식은 분명합니다. 원칙을 지키고, 약속을 지키고, 자리에서 흔들리지 않는 것이 쌓이면 조직과 사람들이 먼저 당신을 찾아옵니다. 조급해하지 않는 편이 좋습니다. 꾸준히 쌓는 사람이 결국 가장 높은 곳에 올라가는 구조입니다.`
                    : `${name}님, 한 가지만 기억하십시오. 명예와 사회적 위치는 말 한마디, 행동 하나에 달려 있습니다. 감정이 격해지는 순간의 발언, 원칙을 조금만 어겨도 괜찮겠지라는 생각이 한순간에 모든 것을 무너뜨립니다. 평소에 지킨 원칙이 위기 때 당신을 지킵니다.`,
                '편관격': isSeong
                    ? `${name}님의 진짜 능력은 편한 환경에서 나오지 않습니다. 역경이 클수록, 경쟁이 치열할수록 더 강해지는 것이 이 사주의 특징입니다. 지금 힘들다면 — 그것은 당신이 맞는 자리에 있다는 신호일 수 있습니다. 버텨내는 것 자체가 당신의 무기입니다.`
                    : `${name}님에게 가능하면 필요한 것이 있습니다 — 충동을 제어하는 구조. 강한 추진력이 방향 없이 폭발하면 사고, 갈등, 법적 문제로 이어질 수 있습니다. 당신을 냉정하게 붙잡아줄 수 있는 멘토나 파트너를 가능하면 옆에 두십시오.`,
                '식신격': isSeong
                    ? `${name}님, 좋아하는 일을 계속 파고드십시오. 재능과 수입이 연결되는 구조입니다. 예술, 요리, 교육, 기획, 창작 — 형태는 무엇이든 자신이 잘하고 즐기는 것을 업으로 삼으면 돈이 따라옵니다. 빠른 수익보다 재능의 깊이를 먼저 쌓으십시오.`
                    : `${name}님, 재능이 있어도 알리지 않으면 없는 것과 같습니다. 지금 당신에게 부족한 것은 능력이 아니라 알리는 방법입니다. 당신이 잘하는 것을 세상에 보여주는 방법을 찾는 것이 지금 가장 시급한 과제입니다.`,
                '상관격': isSeong
                    ? `${name}님의 가장 강력한 무기는 말과 생각입니다. 남들이 당연하다고 받아들이는 것에 "왜?"라고 묻는 능력이 당신을 차별화합니다. 조직 안에서 답답함을 느낀다면 독립적인 환경을 찾으십시오. 비판적 사고가 제약 없이 펼쳐질 때 가장 빛납니다.`
                    : `${name}님, 솔직히 말씀드립니다. 당신의 말이 때로 가장 중요한 관계를 끊습니다. 틀린 말이 아닐 수 있습니다. 그러나 맞는 말을 잘못된 방식으로 하면 결과는 같습니다. 말하기 전에 1초만 더 생각하십시오. 그 1초가 인생을 바꿉니다.`,
                '편인격': isSeong
                    ? `${name}님은 남들이 가지 않은 길에서 나만의 전문성을 만들어내는 사람입니다. 기존의 방식을 답습하지 않는 편이 좋습니다. 독창적으로 파고든 분야에서 타의 추종을 불허하는 깊이가 만들어집니다. 고독하더라도 그 길을 가십시오. 그것이 당신만의 권위가 됩니다.`
                    : `${name}님의 전문성이 다른 사람에 의해 가로막히는 패턴이 반복됩니다. 내가 쌓아온 것을 지키는 것이 먼저입니다. 지식재산권, 계약 문서, 독립적 활동 환경을 가능하면 챙기십시오.`,
                '정인격': isSeong
                    ? `${name}님은 배울수록 가치가 올라가는 사주입니다. 지금 당장의 돈보다 자격증, 학위, 공인된 전문성을 쌓는 것이 장기적으로 훨씬 더 큰 수입을 만들어냅니다. 배움을 멈추지 않는 편이 좋습니다. 노력이 배신하지 않는 구조입니다.`
                    : `${name}님, 배움에 대한 열망이 강한데 결실로 이어지지 않는 경험이 있으실 것입니다. 조심해야 할 것: 스승을 잘못 만나면 시간과 돈을 동시에 잃습니다. 검증되지 않은 교육 투자를 서두르지 않는 편이 좋습니다.`,
                '비겁격': isSeong
                    ? `${name}님, 조직 안에서 답답함을 느끼는 것은 당연합니다. 통제받는 환경보다 스스로 판을 벌이는 환경에서 진가가 나오는 사주입니다. 사업, 프리랜서, 전문직 — 방향은 달라도 독립이 핵심 포인트입니다. 언제 독립할 것인지를 항상 준비하십시오.`
                    : `${name}님, 혼자 다 하려 하지 않는 편이 좋습니다. 강한 자아가 오히려 더 큰 성과를 막고 있을 수 있습니다. 당신이 못하는 것을 잘하는 사람을 파트너로 두는 것이 지금 가장 필요한 전략입니다.`
            };
            ggNarr = '\n\n' + (GG_NARR[gg.geokName] || '');
        }
        ...
        const CAREER_MAP = {
            wood: isStrong ? name+'님은 조직의 규칙이나 남이 정해놓은 틀 안에서는 반이 이하의 능력밖에 발휘하지 못합니다. 직접 기획하고, 개척하고, 처음 만들어나가는 자리에서 진짜 능력이 나옵니다. 창업, 신사업 기획, 1인 전문가, 선도자 — 이런 포지션이 맞습니다. 조직에 있다면 새로운 프로젝트를 먼저 제안하고 이끄는 역할을 찾으십시오.'
                : name+'님은 강한 파트너나 든든한 조직의 지원을 받을 때 진짜 능력이 나옵니다. 혼자보다 팀으로, 독립보다 협력으로 직업적 성취를 쌓아가는 것이 이 사주의 전략입니다. 당신을 알아봐주는 리더 한 명을 찾는 것이 커리어의 분기점이 됩니다.',
            fire: isStrong ? name+'님에게 무대가 없으면 에너지가 안으로 향합니다. 사람들 앞에 서고, 영향력을 발휘하고, 내 아이디어가 세상에 퍼지는 일 — 강연, 컨텐츠 창작, 세일즈, 리더십, 브랜딩이 천직입니다. 혼자 책상에 앉아 조용히 하는 일은 장기적으로 맞지 않습니다.'
                : name+'님은 나를 알아봐 주는 환경에서 열정이 폭발합니다. 아직 그런 환경을 못 찾았다면, 지금이 바로 그것을 찾아야 할 시기입니다. 직업 선택보다 직장 환경과 상사 선택이 더 중요합니다.',
            earth: isStrong ? name+'님은 장기적인 신뢰 관계를 기반으로 하는 직업에서 가장 빛납니다. 부동산, 금융, 컨설팅, 교육, 의료 — 오랫동안 같은 자리를 지키며 신뢰를 쌓아가는 직업이 맞습니다. 빠른 변화가 잦은 직종보다 안정적이고 깊이 있는 전문성을 쌓을 수 있는 분야를 선택하십시오.'
                : name+'님에게 직업적 안정감은 선택이 아닌 필수 조건입니다. 수입이 들쭉날쭉한 환경은 실력과 상관없이 성과를 떨어뜨립니다. 기반을 먼저 만들고 그 위에서 도전하십시오.',
            metal: isStrong ? name+'님은 전문성과 원칙이 인정받는 분야에서 타의 추종을 불허합니다. 법, 의학, 회계, IT, 정밀 기술, 컨설팅 — 높은 기준이 요구되는 분야가 맞습니다. 하나의 전문 분야를 깊이 파고드는 전략이 넓게 아는 전략을 압도합니다.'
                : name+'님은 능력이 있어도 그것을 인정받을 환경을 못 찾아 헤매는 시기가 길 수 있습니다. 환경 탓이 아니라 환경을 바꾸는 것이 답입니다. 지금의 직장이 당신의 기준을 알아봐 주지 않는다면, 그곳에서 소진되지 않는 편이 좋습니다.',
            water: isStrong ? name+'님의 직업적 강점은 통찰력과 전략적 사고입니다. 데이터 분석, 전략 기획, 투자, 연구, 컨설팅, 작가 — 깊이 생각하고 패턴을 읽어내는 능력이 핵심 경쟁력인 분야가 맞습니다. 단, 실행력을 의식적으로 끌어올리지 않으면 기회를 놓치는 패턴이 반복됩니다.'
                : name+'님은 오랜 시간 준비하고 축적한 것이 한 번에 빛을 발하는 구조입니다. 조급해하지 않는 편이 좋습니다. 지금 쌓고 있는 것이 가능하면 결실이 됩니다.'
        };

        const LOVE_MAP = {
            wood: isStrong ? name+'님은 연애와 결혼에서도 자신만의 방식을 고집하는 편입니다. 상대방이 내 방향성을 지지하고 함께 성장하려는 사람이어야 관계가 오래 갑니다. 내 길을 막거나 통제하려는 상대와는 가능하면 갈등이 생깁니다. 파트너를 선택할 때 "이 사람이 나의 성장을 응원하는가"를 가장 먼저 보십시오.'
                : name+'님은 관계에서 헌신적이지만 그만큼 상처도 깊게 받는 구조입니다. 나를 지워가며 관계를 유지하는 패턴을 조심하십시오. 좋은 관계는 내가 나로 있을 수 있는 관계입니다.',
            fire: isStrong ? name+'님은 연애에서 강렬하고 빠르게 타오릅니다. 그만큼 빠르게 식을 수도 있습니다. 처음의 뜨거운 감정이 식었을 때 진짜 감정이 무엇인지 스스로 물어보십시오. 장기적인 관계를 위해서는 상대방에게 공간을 주고 감정을 강요하지 않는 훈련이 필요합니다.'
                : name+'님은 연애에서 상대방의 인정과 관심이 특히 필요합니다. 이것을 주는 사람을 찾는 것이 중요하지만, 그 필요 때문에 맞지 않는 관계에 오래 머무는 패턴도 조심하십시오.',
            earth: isStrong ? name+'님은 신중하게 관계를 시작하고, 한번 마음을 주면 끝까지 지키는 스타일입니다. 이것이 가장 큰 강점이자 동시에 상처를 오래 품는 이유이기도 합니다. 관계에서 충분히 검증하는 것은 좋지만, 너무 오래 확인하다가 기회를 놓치지 않는 편이 좋습니다.'
                : name+'님은 안정적인 관계에 대한 열망이 강합니다. 이 열망이 때로 맞지 않는 관계에도 매달리게 만듭니다. 관계의 안정보다 나 자신의 안정을 먼저 찾는 것이 순서입니다.',
            metal: isStrong ? name+'님은 관계에서도 높은 기준을 적용합니다. 이상형이 구체적이고, 상대방에게 기대하는 것도 명확합니다. 이것이 잘 맞는 상대를 만나면 최고의 파트너십이 되지만, 현실의 사람에게 너무 높은 기준을 요구하면 외로워집니다. 완벽한 사람보다 함께 성장할 수 있는 사람을 찾으십시오.'
                : name+'님은 연애에서 이상과 현실 사이에서 갈등하는 경우가 많습니다. 충분히 좋은 사람을 완벽하지 않다는 이유로 놓치지 않는 편이 좋습니다.',
            water: isStrong ? name+'님은 상대방을 깊이 읽는 능력이 있습니다. 상대가 말하지 않은 것도 감지하는 예민함이 있습니다. 그러나 이 감각이 지나치면 관계에서 지나치게 분석하고, 상대의 작은 변화에 불안해하는 패턴이 생깁니다. 직관을 믿되, 결론을 너무 빨리 내리지 않는 편이 좋습니다.'
                : name+'님은 관계에서 신중하고 천천히 마음을 여는 스타일입니다. 처음에는 느려 보여도 한번 깊어진 관계는 오래 갑니다. 마음을 여는 데 시간이 걸리는 자신을 탓하지 않는 편이 좋습니다. 그것이 당신의 방식입니다.'
        };
    ...
    const gongmangText = (() => {
        ...
        return `<div style="background:rgba(231,76,60,0.06);border-left:3px solid rgba(231,76,60,0.4);padding:14px 18px;border-radius:0 10px 10px 0;margin-top:16px;">
            ...
            <p style="font-size:13px;color:#ddd;line-height:1.85;margin:0;">${name}님의 <b>${gmPillarNames}</b>에 공망이 걸려 있습니다. <b>${pillarAreas}</b> 영역에서 ${branchMeanings}에 대한 갈망이 반복됩니다. 이 기운이 강하게 들어오는 대운·세운에서 중요한 결정을 서두르지 않는 편이 좋습니다.</p>
        </div>`;
    })();
    ...
    return `<div class="report-chapter chapter-start">
        ...
        <div style="background:rgba(199,167,106,0.07);border-left:4px solid var(--gold);padding:20px 22px;border-radius:0 12px 12px 0;margin:24px 0;">
            <div style="font-size:11px;color:var(--text-dim);margin-bottom:12px;letter-spacing:2px;">✦ ${name}님의 사주 — 종합 통변</div>
            ${lifeNarr.split('\n\n').map((para,i) => {
                if(!para.trim()) return '';
...
function buildChapter2_Wuxing(data) {
    ...
    return `<div class="report-chapter">
        ...
        <div style="background:rgba(199,167,106,0.06);border-radius:10px;padding:14px 18px;margin-bottom:16px;border-left:3px solid var(--gold);">
            ...
            <span style="font-size:15px;font-weight:700;color:var(--gold);">${maxKr} 우세</span> &nbsp;·&nbsp; <span style="font-size:13px;color:#bbb;">${name}님의 판단·관계·재물 방식의 중심</span>
        </div>
            ...
            ${(()=>{
                ...
                const HEALTH_DESC = {
                    wood: { col:'#4fc3a1', title:'간·담낭·신경계',
                        ...
                        weak:`${name}님의 사주에서 목(나무) 기운이 약하게 자리잡고 있습니다. 이 경우 몸에서 나타나는 신호는 만성 피로, 결단력 저하, 무기력감입니다. 간과 신경계를 꾸준히 챙겨야 합니다. 규칙적인 수면과 기상 시간을 지키는 것이 가장 중요합니다. 봄 제철 채소를 충분히 섭취하고, 스트레칭과 가벼운 유산소를 꾸준히 하십시오.`
                    },
                    fire: { col:'#ff7043', title:'심장·심혈관·수면',
                        strong:`${name}님의 사주에는 화(불) 기운이 강하게 집중되어 있습니다. 이 기운이 넘칠 때 가장 먼저 심장과 혈관이 신호를 보냅니다 — 가슴 두근거림, 불면증, 안면홍조, 그리고 감정의 급격한 기복입니다. 여름(6~8월)에 이 증상이 심화됩니다. 카페인과 에너지드링크를 멀리하십시오. 취침 전 2시간은 자극적인 콘텐츠를 피하고, 혈압을 정기적으로 체크하는 습관을 들이십시오. 명상이나 심호흡이 이 사주에 가장 좋은 건강 습관입니다.`,
                        weak:`${name}님의 사주에서 화(불) 기운이 약합니다. 열정이 쉽게 꺼지고, 순환이 잘 안 되며, 손발이 차가운 증상이 나타날 수 있습니다. 적당한 유산소 운동으로 심폐 기능을 꾸준히 키우십시오. 따뜻한 음식을 챙기고, 혼자 있는 시간보다 사람들과 어울리는 시간이 이 사주의 건강 에너지를 충전합니다.`
                    },
                    earth: { col:'#ffca28', title:'위장·소화기·면역',
                        strong:`${name}님의 사주에는 토(흙) 기운이 강하게 집중되어 있습니다. 이 기운이 넘칠 때 위장과 소화기에 신호가 옵니다 — 위염, 역류성 식도염, 소화불량, 그리고 과도한 걱정과 생각으로 인한 식욕 저하입니다. 스트레스가 쌓이면 먹는 것으로 푸는 패턴도 조심해야 합니다. 규칙적인 식사 시간을 지키는 것이 첫 번째 건강 습관입니다. 야식과 폭식을 피하고, 식후 30분 산책을 생활화하십시오.`,
                        weak:`${name}님의 사주에서 토(흙) 기운이 약합니다. 면역력이 쉽게 떨어지고 피로 회복이 느린 경향이 있습니다. 규칙적인 생활 리듬이 이 사주의 건강을 지키는 가장 강력한 방패입니다. 고구마·대추·단호박 등 자연 단맛 식품을 챙기십시오.`
                    },
                    metal: { col:'#b0bec5', title:'폐·호흡기·피부',
                        strong:`${name}님의 사주에는 금(쇠) 기운이 강하게 집중되어 있습니다. 이 기운이 넘칠 때 폐와 호흡기에 신호가 옵니다 — 기침, 피부 트러블, 변비, 그리고 슬픔이나 우울감이 신체 증상으로 나타납니다. 가을(9~11월)에 이 증상이 집중됩니다. 미세먼지 심한 날 마스크는 필수입니다. 수영이나 유산소 운동으로 폐활량을 키우고, 감정을 억누르지 말고 표현하는 것이 폐 건강에 직결됩니다.`,
                        weak:`${name}님의 사주에서 금(쇠) 기운이 약합니다. 결단력이 흔들리고 호흡기 감염에 취약한 경향이 있습니다. 도라지·무·배 등 호흡기를 강화하는 음식을 자주 드시고, 심호흡 운동을 꾸준히 하십시오.`
                    },
                    water: { col:'#64b5f6', title:'신장·호르몬·체력',
                        strong:`${name}님의 사주에는 수(물) 기운이 강하게 집중되어 있습니다. 이 기운이 넘칠 때 신장과 호르몬 시스템에 신호가 옵니다 — 허리 통증, 부종, 탈모, 그리고 만성 피로와 두려움이 신체 증상으로 나타납니다. 겨울(12~2월)에 이 증상이 심화됩니다. 하루 수면 7시간 이상을 철저히 지키고, 물 2L 이상 섭취를 습관화하십시오. 검은콩·흑임자·미역을 꾸준히 드십시오.`,
                        weak:`${name}님의 사주에서 수(물) 기운이 약합니다. 체력 저하와 집중력 감소가 쉽게 나타날 수 있습니다. 충분한 수분 섭취와 규칙적인 수면이 이 사주의 건강을 지키는 핵심 포인트입니다. 두려움과 불안이 신체로 연결되지 않도록 명상이나 일기 쓰기로 감정을 정리하십시오.`
                    }
                };
        ...
        <div style="background:rgba(199,167,106,0.05);border-radius:12px;padding:22px;margin:24px 0;border:1px solid rgba(199,167,106,0.1);">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
                ...
                <div style="background:rgba(255,255,255,0.04);border-radius:8px;padding:12px;"><div style="font-size:11px;color:#64b5f6;margin-bottom:6px;">수 기운 직업 환경</div><p style="font-size:12.5px;color:#bbb;line-height:1.8;margin:0;">수 기운이 활성화된 환경: 연구·분석·IT·금융·철학·심리·유통·무역 분야. 깊이 있는 사고와 전략이 필요한 환경이 수 기운 보충에 최적입니다.</p></div>
...
function buildChapter3_Sipseong(data) {
    ...
    const sipDB = {
        ...
        '식신':'**IT·콘텐츠·디자인·기획**처럼 손끝 산출물이 평가로 바로 붙는 일에서 살아납니다. 재능이 곧 수입이 되도록 **‘판매 가능한 산출물’ 한 종류**만 먼저 고르십시오.',
...
function buildChapter4_Wealth(data) {
    ...
    const wealthDesc = wealthType === 'active'
        ? `${name}님은 현금이 손을 타고 흐를 때 힘이 납니다. 월급만으로는 속도가 안 맞을 수 있습니다. 그건 욕심이 아니라 구조입니다. **테마 성격 자산·부업 파이프라인은 세 개 넘기지 마십시오.** 길운 월에만 단기 매매·공모주 승부를 허용하십시오.`
        : `${name}님은 ‘이름·전문성·신뢰’가 쌓일수록 돈이 따라붙는 축입니다. 한탕보다 **배당형·대형 우량 코어**와 반복 계약이 맞습니다. **브랜딩 한 줄·현금흐름 표**를 먼저 고정하십시오.`;

    const wealthCaution = wealthType === 'active'
        ? `속도가 곧 함정입니다. 파생·고레버리지 **한 번**이 전부를 가져갈 수 있습니다. **손실 한도·보증 거절**을 문장으로 적어 지갑 옆에 두십시오.`
        : `돈을 쫓으면 도망갑니다. 가격표를 먼저 올리십시오. **고변동 자산 비중 10% 미만**을 지키고, 분기마다 **증명 가능한 성과** 한 가지만 대외에 보내십시오.`;
    ...
    const daewunRows = rows.slice(0, 8).map((r, i) => {
        ...
        const intro = phase==='open'
            ? `${ganKr}${jiKr}(${age}~${endAge}세) — 흐름이 열립니다. **테마 성격 자산·고변동 파이프라인**에서 리스크 대비 수익이 열리나, **청산 기준·손실 한도** 없이는 승부하지 마십시오.`
            : phase==='guard'
            ? `${ganKr}${jiKr}(${age}~${endAge}세) — 새는 구간입니다. **레버리지·비공식 투자·보증**부터 잠그고, **대형 우량·배당형 펀드**로 코어를 묶으십시오.`
        ...
        const openActionPool = [
            ...
            `${ganKr}${jiKr}(${age}~${endAge}세)에서는 천간·지지가 동시에 당겨지기 쉬우니, **메인 수익 파이프 한 가지**만 고정하고 나머지는 자동이체로 묶으십시오.`,
            `${age}~${endAge}세 **${ganKr}${jiKr}**에는 기회 신호가 빠릅니다. **야간 주문·감정 매매**를 막기 위해 증권·거래 앱 알림을 주간 리포트로만 바꾸십시오.`,
        ...
        const ganLine = ganSip ? ('겉 일정·대외 과제 쪽에는 ' + sipToIndustryWorkStyle(ganSip) + ' 흐름이') : '천간 쪽 흐름이';
        const jiLine = jiSip ? ('현장·생활 토대에는 ' + sipToIndustryWorkStyle(jiSip) + ' 흐름이') : '지지 쪽 흐름이';
    ...
    return `<div class="report-chapter">
        ...
        ${buildDomainSummaryTable({
            boxTitle: name + '님을 위한 재물 전략',
            keyword:'현금흐름 · 리스크 한도 · 실행 타이밍',
            route:'수익축 2~3개 집중 + 월 단위 점검 고정',
            caution:'보증·충동 투자·감정 계약을 즉시 중단하십시오'
        })}
        <p class="ch-text">재물은 감이 아니라 리듬입니다. 벌 때와 줄 때를 구분하지 못하면 같은 실수가 반복됩니다. **용신 구간은 고변동·테마 성격 자산까지 공격**, 기신 구간은 **지수·배당형·현금 코어**로만 수비하십시오.</p>
        ...
        <div style="background:rgba(255,255,255,0.03);border-radius:12px;padding:20px;margin:20px 0;">
            ...
            <div style="display:flex;flex-direction:column;gap:10px;">
                <div style="background:rgba(255,255,255,0.04);border-radius:8px;padding:14px;">
                    ...
                    <p style="font-size:13px;color:#bbb;line-height:1.88;margin:0;">기신이 두꺼울 때는 판단이 흐립니다. 투자·보증·동업 확장은 **원칙적으로 보류**하십시오. 큰 결정은 **72시간·서면 조건** 없이 내리지 마십시오.</p>
                </div>
                <div style="background:rgba(255,255,255,0.04);border-radius:8px;padding:14px;">
                    <div style="font-size:12px;font-weight:700;color:var(--gold);margin-bottom:8px;">용신 기운이 강한 시기 — 이때 움직이십시오</div>
                    <p style="font-size:13px;color:#bbb;line-height:1.88;margin:0;">용신·희신이 겹치면 **도약 구간**입니다. 미리 준비한 사람만 올라탑니다. **현금·건강·법적 서류** 세 가지를 문 앞에 두십시오.</p>
                </div>
            </div>
        </div>
        <p class="ch-text" style="margin-top:16px;">돈은 빠르게 번 것보다 오래 남는 쪽이 이깁니다. ${name}님의 리듬은 ${isStrong ? '**나눠 넣고 쉬는 날을 달력에 고정**' : '**파트너와 주간 점검 한 번**'}하십시오.</p>
    </div>`;
}
...
function buildDaewunLoop(data) {
    ...
    rows.forEach((r, idx) => {
        ...
        const CAREER_JI = {
            '子':'기획·분석이 빛납니다. **PRD·데이터로 결론**을 내리십시오.',
        ...
        // 애정 조언
        const loveAdv = sc>=2 ? '**매칭·소개 서비스·메시지**에서 접점이 열립니다. **기준 한 줄**을 프로필에 먼저 쓰십시오.' : sc>=0 ? '안정 국면입니다. **영상통화 1회 후 대면**·대화 시간 고정이 관계를 지킵니다.' : '오해 비용이 큽니다. **메시지·단체 대화**는 증빙으로 남기고 감정 결정은 유예하십시오.';
...
function buildChapter5_Career(data) {
    ...
    const careerDB = {
        officer:{label:'관리형 — 조직과 명예', desc:'조직·평가·직책 축이 두껍습니다. 규칙과 위계 안에서 빛납니다. 그건 순응이 아니라 **책임이 곧 브랜드**인 구조입니다. **거버넌스·규정 준수·금융 리스크** 축에서 승진 트랙을 타십시오. **권한 범위를 문서로** 먼저 고정하십시오.', jobs:'프로덕트 오너·리스크·법무·금융·헬스케어 규정·대기업 임원 트랙'},
        creator:{label:'창조형 — 재능과 표현', desc:'기획·산출·표현 축이 두껍습니다. 남의 시스템에 끼면 소진됩니다. **대외 브랜딩**으로 **내 이름이 붙은 산출물**이 곧 돈입니다. **콘텐츠·업무 자동화**로 파이프라인을 줄이십시오. **주간 공개 일정**을 달력에 박으십시오.', jobs:'기획·영상·디자인·마케터·개발·운영·대외 커뮤니케이션'},
        expert:{label:'전문가형 — 지식과 자격', desc:'학습·자격·연구 축이 두껍습니다. 따라오기 어려운 전문성이 자산입니다. **데이터·연구·자격**으로 장벽을 쌓으십시오. 배움을 멈추면 나이가 들수록 약해집니다.', jobs:'의사·변호사·회계·컨설턴트·연구원·분석·보안 설계'},
        independent:{label:'독립형 — 자율과 경쟁', desc:'동료·경쟁·자기주도 축이 강하면 남 밑이 답답합니다. **자율 100% 자리**가 아니면 반토막입니다. **사업 설계·영업·단기 매매**에서 승부하십시오.', jobs:'사업가·1인사·영업·부동산·자영업·커뮤니티 운영'}
    };
...
function buildChapter6_Love(data) {
    ...
    const meetTiming = name + '님 인연은 운이 열릴 때만 오는 것이 아닙니다. **매칭·소개 서비스**와 **메시지·비대면 접점**이 열려 있어야 스침이 아니라 접점이 됩니다. 기운이 맞는 해·달에만 **직무·취향이 맞는 소규모 커뮤니티** 참여를 늘리십시오.';

    return `<div class="report-chapter">
        <h3 class="ch-title">[ 06. 애정·인연 ] — 마음이 머무는 자리 — 인연과 사랑의 결</h3>
        ${buildDomainSummaryTable({
            boxTitle: name + '님을 위한 애정 전략',
            keyword:'매칭 채널 · 메시지 리듬 · 리스크 관리',
            route:'소개·매칭 필터 정리 후 비대면 확인 1회를 고정하십시오',
            caution:'감정 최고점·투자·지분 이야기가 겹친 날의 관계 결정은 유예하십시오'
        })}
        <p class="ch-text">애정은 횟수가 아니라 **리듬**입니다. 감정이 최고로 올라간 날에 서명하지 마십시오.</p>
        <p class="ch-text">이성운의 핵은 “누구를 만나느냐”가 아니라 **나는 어떤 패턴으로 붙고 떨어지느냐**입니다. 패턴을 알면 같은 상처를 반값에 삽니다.</p>
...
function buildZiWeiDestinyBlueprintSection(data) {
    ...
    var p2 = '<p style="margin:0 0 14px;line-height:1.92;color:#ddd;font-size:13.5px;">'
        ...
        + C.f1 + ' ' + G.f2 + ' '
        + '<span style="color:#9dd3ff;font-size:11px;font-weight:700;letter-spacing:0.06em;">[실행]</span> '
        + '**고변동 상품**은 손실 한도 아래로만 두고, **대형 우량·배당형 펀드**로 월 현금흐름의 다음 층을 먼저 쌓으십시오.</p>';
    var p3 = '<p style="margin:0;line-height:1.92;color:#ddd;font-size:13.5px;">'
        ...
        + T.o + ' ' + N.o + ' '
        + '<span style="color:#9dd3ff;font-size:11px;font-weight:700;letter-spacing:0.06em;">[실행]</span> '
        + '**매칭·메시지 채널**에는 직무·가치관 필터를 먼저 적고, 대면은 비대면으로 한 차례 확인한 뒤로 미루십시오.</p>';
    return '<div id="sec-ziwei-appendix" class="report-chapter chapter-start appendix-ziwei" style="padding-top:8px;margin-bottom:8px;">'
        + '<div style="font-size:12px;color:#9dd3ff;font-weight:900;letter-spacing:0.06em;margin-bottom:10px;line-height:1.55;">[별첨] 동양의 점성술로 보는 정밀 심리 분석 — 자미두수</div>'
...
function buildPart1_Story(data){
    ...
    html += '<p class="ch-text" style="font-size:13.5px;color:var(--text-soft);line-height:1.85;margin:0 0 14px;">사주 원국과 대운의 흐름을 하나의 스토리로 통합하여 삶의 궤적을 설명합니다.</p>';
    html += '</div>';
    return html;
}
...
function buildChapter6_SeYun(data){
    ...
    return '<div class="report-chapter chapter-start"><h3 class="ch-title" style="font-family:\'Noto Serif KR\',\'Nanum Myeongjo\',\'Batang\',serif;">'+h3t+'</h3>'
...
function buildChapter8_NextDaewun(data){
    ...
    // 현재 나이 기준 다음 대운 찾기
    var curAge=new Date().getFullYear()-parseInt((data.birthDate||'1990').substring(0,4));
    var nextDW=null;
    ...
    var DWNEXT={wood:name+'님 다음 대운은 목(木)입니다. 시작·학습·이사에 기운이 실립니다. 판을 여러 개 깔면 체력이 먼저 갑니다. **새 도전은 연 두 개 이하**로 묶으십시오.',fire:name+'님 다음 대운은 화(火)입니다. 노출·성취·이름이 커집니다. 번영과 함께 건강·재무 방어가 무너지기 쉽습니다. **수면·지출 상한선**을 먼저 박으십시오.',earth:name+'님 다음 대운은 토(土)입니다. 안정·축적·완성에 무게가 실립니다. 서두를 필요는 적습니다. **자산·관계를 정리해 결실만** 거두십시오.',metal:name+'님 다음 대운은 금(金)입니다. 전문성·결단·잘라내기가 강해집니다. 한 분야 깊이가 배수로 붙습니다. **핵심 한 줄에만 시간을 몰아** 붙이십시오.',water:name+'님 다음 대운은 수(水)입니다. 통찰·정보·인맥이 자본이 됩니다. 말보다 기록이 이깁니다. **네트워크를 문서와 숫자로**만 관리하십시오.'};
...
const FORTUNE_TEXT_DB = {
    sipseong: {
        ...
        '편재': [
            '대형 자금·사업 기회가 동시에 진입합니다.',
            '외부 자본과 거래 스케일이 커집니다.',
            '빠른 회전의 수익 기회가 열립니다.',
            '고변동·테마 성격 자산·단기 파생에서 **리스크 대비 수익**이 커집니다.'
        ],
        '정재': [
            '현금흐름과 고정수익을 다지는 구간입니다.',
            '월 단위 수익 구조를 재정렬해야 합니다.',
            '지출 통제와 누적 자산 축적이 핵심입니다.',
            '배당형·월세·정기 수익 파이프라인으로 **재무 안정 코어**를 잡으십시오.'
        ],
        '편관': [
...
var strat = s>=2 ? STRAT_GOOD.join('<br>') : s>=0 ? STRAT_MID.join('<br>') : STRAT_BAD.join('<br>');
        ...
        (function() {
            ...
            (function(){
                ...
                txt += '<p style="font-size:13px;color:#bbb;line-height:1.85;margin:0 0 10px;">위 그래프는 '+name+'님의 대운 에너지 곡선입니다. 용신/희신 기운이 강한 대운은 <span style="color:#c7a76a;font-weight:700;">황금색(🌟)</span>으로 표시되고, 기신/구신 기운은 <span style="color:#e74c3c;font-weight:700;">빨간색(❌)</span>으로 표시됩니다. 곡선이 높을수록 길한 시기, 낮을수록 주의가 필요한 시기입니다.</p>';
        ...
        (function(){
            ...
            function gc(s){return s>=3?'#c7a76a':s>=1?'#00C853':s===0?'#888':s>=-2?'#ff9800':'#e74c3c';}
            function gb(s){return s>=3?'🌟 대길':s>=1?'✦ 길':s===0?'— 평':s>=-2?'⚠ 주의':'❌ 흉';}
            var h2=`<div class="report-chapter"><h3 class="ch-title">대운 80년 — 10년 단위 완전 해부</h3><p class="ch-text">${name}님의 대운 흐름 — 각 10년 구간별 기회와 조심 시기를 아래에서 확인하십시오.</p>`;
            daeunData.forEach(function(dy,idx){
                var gz=dy.getGanZhi();var g=gz[0];var j=gz[1];
                var age=dy.getStartAge()-1;
                var endAge=idx<daeunData.length-1?(daeunData[idx+1].getStartAge()-1):age+10;
        ...
        (function(){
            ...
            for(var m4=1;m4<=12;m4++){
                ...
                var mTail=[
                    ...
                    '매칭·메시지 채널은 **필터 문장**을 먼저 고치는 달입니다.',
