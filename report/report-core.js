...
function buildPersonalPortrait(data) {
    ...
    // ─── 12개 시기 시나리오 (120세 기준 한 사람의 인생 스토리) ───
    var t1 = periodTone(0, 7);
    var t2 = periodTone(8, 13);
    var t3 = periodTone(14, 19);
    var t4 = periodTone(20, 29);
    var t5 = periodTone(30, 39);
    var t6 = periodTone(40, 49);
    var t7 = periodTone(50, 59);
    var t8 = periodTone(60, 69);
    var t9 = periodTone(70, 79);
    var t10 = periodTone(80, 89);
    var t11 = periodTone(90, 99);
    var t12 = periodTone(100, 120);

    var s1 = nmDnim(name) + '은 ' + birthScene + '. ' + (firstImpression ? '그 시절의 ' + nmDnim(name) + '은 어른들의 눈에 “' + firstImpression + '” 같은 인상이었을 가능성이 큽니다. ' : '')
        + tonePhrase(t1,
            '0~7세 사이에 큰 사고나 떠들썩한 사건은 거의 없었고, 가족의 따뜻한 시선이 ' + nmEulReul(name) + ' 든든히 받쳐 줍니다. 그때 받은 작은 칭찬 한 줄이 사실 평생 가는 자존감의 가장 깊은 바닥에 깔립니다.',
            '0~7세 사이에 이사·전학·부모님 직장 변화 같은 사건이 한두 번 따라왔을 수 있어요. 안정보다 변화에 먼저 적응해야 했던 그 시기 덕분에 — ' + nmDnim(name) + '은 또래보다 일찍 어른의 표정을 읽는 법을 익히셨습니다.',
            '0~7세 사이는 큰 굴곡 없이 잔잔하게 흘러간 시기였지만, 그 잔잔함 속에서 어린 ' + nmDnim(name) + '은 “나는 어떤 사람일까”라는 물음을 이미 마음 한구석에 품기 시작하셨습니다.');

    var s2 = '초등학교에 들어가신 8~13세의 ' + nmDnim(name) + '은 또래 사이에서 “조용하지만 속이 깊은 아이”로 기억되셨을 가능성이 큽니다. 친구들이 떠들썩하게 어울릴 때도 한 발 떨어져 그 흐름을 지켜보는 편이었고, 그 자리에서 또래보다 한 박자 일찍 “사람의 마음”에 눈을 뜨셨어요. '
        + tonePhrase(t2,
            '학교 안에서 한두 분 — 선생님이나 친한 친구 — 이 ' + nmEulReul(name) + ' 알아봐 주셨고, 그 인정 한 줄이 청소년기로 가는 자존감의 뼈대가 됩니다.',
            '학교에서 한두 번 깊은 마찰이나 외로움을 겪으셨을 수 있어요. 그때의 외로움이 사실 ' + nmDnim(name) + '만의 색을 만든 첫 출발점이 됩니다.',
            '큰 사건은 없었지만 작은 일들이 차곡차곡 마음에 쌓이며 — 어떤 과목·어떤 친구·어떤 분위기가 ' + nmEulReul(name) + ' 가장 ' + nmDnim(name) + '답게 만드는지를 어렴풋이 알기 시작하시는 시기예요.');

    var s3 = '14~19세 — ' + nmUi(name) + ' 청소년기는 자아의 첫 출발선입니다. 겉으로 보기엔 ' + (firstImpression || '진중한 분') + '의 이미지가 굳어 가지만, 속내는 한참 달랐어요. ' + (inSelfNote || '겉과 속이 다른 이중성을 일찍부터 안고 사신 시기') + '. '
        + tonePhrase(t3,
            ...
            '진로 선택을 두고 부모님과 한 번 깊게 부딪치셨을 가능성이 큽니다. 가고 싶은 길과 부모님이 원하시는 길이 어긋났을 수 있고, 첫 사랑이나 첫 친한 친구 관계에서도 한 번 크게 다치셨을 수 있어요. 그 흔들림이 ' + nmUi(name) + ' 세상을 보는 눈을 깊게 만듭니다.',
            '큰 풍파 없이도 “나는 어떤 사람인가”라는 질문을 일찍부터 마음에 품기 시작하시고, 답을 빨리 내리지 않으셔도 그 질문 자체가 평생 ' + nmEulReul(name) + ' 받치는 축이 됩니다.');

    var s4 = '20~29세 — ' + nmDnim(name) + '이 사회로 나가시는 첫 10년입니다. ' + nmUi(name) + ' 가장 큰 강점이 “' + strengthRaw + '”이라는 것이 이 시기에 처음 시장에서 확인됩니다. ' + nmDnim(name) + '은 ' + dominantSip + '이세요. '
        + tonePhrase(t4,
            '첫 직장·첫 자취·첫 큰 연애 — 이 시기의 굵직한 결정들이 의외로 ' + nmEulReul(name) + ' 받쳐 줍니다. 작은 성공 하나가 “나는 이 길로 가도 되겠구나”라는 확신을 키워 주고, 30대 초반 자리잡기의 토양이 됩니다.',
            '첫 사회 진입에서 한 번 크게 흔들리시거나, 첫 직장이 오래 가지 못할 수 있어요. 그때의 학습이 사실 30대에 진짜 자기 자리를 찾을 때의 가장 큰 자산이 되니, 그 흔들림은 부끄러운 일이 아닙니다.',
            '큰 도약보다 작은 결정들이 차곡차곡 쌓이는 시기예요. 한 번에 자리잡지는 못하셔도, 이 10년 동안 만난 사람·해 본 일들이 30대의 진짜 정착을 위한 재료가 됩니다.');

    var s5 = '30~39세 — ' + nmUi(name) + ' 본 모습이 사회에서 본격적으로 펼쳐지는 가장 활발한 10년입니다. 직업·결혼·이주·창업·자녀 같은 굵직한 결정이 한꺼번에 몰려와요. ' + decisionLine + '. '
        + tonePhrase(t5,
            '30대 초반에 한 번의 큰 도약이 따라오고, 결혼·이직·집 마련 가운데 하나가 자연스럽게 결정됩니다. 그 결정이 평생의 큰 줄기를 만들고, 30대 후반에는 ' + nmUi(name) + ' 이름이 적힌 통장·자산 가운데 하나가 자리를 잡습니다.',
            '결혼이 30대 후반으로 늦춰지거나 한 번 깊은 학습을 거쳐 진짜 짝을 만나시는 흐름일 수 있어요. 직장에서도 30대 중반까지 “이게 정말 내 자리인가”를 자주 묻게 되는 시기지만, 그 학습이 30대 후반의 단단한 안착을 가능하게 합니다.',
            '화려한 도약보다 한 걸음씩 자기 자리를 다지는 흐름이고, 30대 중반에야 비로소 ' + nmDnim(name) + '만의 색이 시장에서 인정받기 시작합니다. 그때가 사실상의 “진짜 시작”이에요.');

    var s6 = '40~49세 — 인생의 중간점이자 가장 굵직한 재정의의 시기입니다. 사회적으로는 가장 활발하지만, 동시에 ' + nmIGa(name) + ' 자주 부딪히시는 벽 — “' + weaknessRaw + '”이 이 시기에 가장 또렷이 모습을 드러내요. ' + (emptyNote ? emptyNote + '. ' : '') + '강한 기질의 뒷면에는 늘 그 그늘이 따라오니, 약점이라며 부끄러워하실 일은 아니랍니다. '
        + tonePhrase(t6,
            '평판이 한 번에 결실로 돌아오고, 40대 후반에는 ' + nmUi(name) + ' 이름이 ' + (careerRaw ? careerRaw + ' ' : '') + '분야 안에서 또렷한 한 줄이 됩니다.',
            '40대 중반에 한 번의 큰 멈춤(번아웃·건강 신호·관계 정리)이 찾아올 가능성이 큽니다. 그 멈춤이 결국 후반전을 다시 짜는 가장 중요한 출발점이 되어 줘요.',
            '큰 영광보다 안정과 깊이가 중심이 되는 흐름이고, 40대 동안 차곡차곡 쌓아 둔 것이 50대 도약의 골격이 됩니다.');

    var s7 = '50~59세 — 인생에서 가장 어깨가 무거운 시기에 들어옵니다. 자녀가 있다면 그 진로 결정, 부모님의 건강, 본업의 마무리·전환, 재정의 큰 줄기가 한꺼번에 몰려와요. 관계 안에서 ' + nmDnim(name) + '은 ' + relationLine + '. '
        + tonePhrase(t7,
            '평생 일군 평판이 한꺼번에 결실로 돌아오는 흐름이라, 50대 후반에는 “이름이 적힌 한 줄” — 한 권의 책, 한 사람의 제자, 한 채의 집 — 그 결과가 손에 잡힙니다.',
            '건강과 가족을 동시에 챙기셔야 하는 시기예요. 무리한 확장보다 회복과 정리가 우선이고, 50대 초반의 건강 검진은 절대 미루지 마십시오. 한 가지 만성 신호가 보이면 그때부터 “관리의 습관”으로 넘어가셔야 합니다.',
            '안쪽보다 바깥에서 새 무대가 한 번 더 열리는 흐름이라, 50대에 시작한 일이 60대의 명함이 될 수 있어요. 30~40대보다 50~60대에 더 빛나시는 분이라, 너무 일찍 “다 끝났다”고 마음 놓지 마십시오.');

    var s8 = '60~69세 — 자녀(있다면)가 독립하고, 회사도 자리를 정리하면서 ' + nmDnim(name) + '이 “진짜 ' + nmUi(name) + ' 시간”을 회복하시는 시기예요. ' + (careerRaw ? '오랫동안 ' + careerRaw + ' 분야에서 다듬어 오신 안목이, 이제는 “일”이 아니라 “취미·후학 양성·작은 공동체” 같은 다른 모양으로 풀려나갑니다. ' : '') + '평생 “나만의 색을 인정받는 자리”를 가장 중요하게 여겨 오신 분이라, 이 시기엔 그 색이 처음으로 자유로워집니다. '
        + tonePhrase(t8,
            '이 10년 안에 늦은 인정 — 상·인터뷰·요청 — 이 한 번 찾아오고, ' + nmEulReul(name) + ' 더 깊은 자리로 안내합니다.',
            '60대 중반에 한 번의 큰 헤어짐(부모님·친구·동료)이 따를 수 있어요. 그 슬픔이 ' + nmUi(name) + ' 노년의 깊이를 결정짓는 분기점이 됩니다.',
            '큰 사건 없이 잔잔히 흘러가는 흐름이지만, 이 시기에 “일과 무관한 새 관심사 하나”를 만드시는 분과 그렇지 않은 분이 70대에서 갈립니다.');

    var s9 = '70~79세 — 외부의 책임이 한 발 멀어지고, ' + nmDnim(name) + '의 진짜 본 모습이 가장 자연스럽게 드러나는 시기입니다. 일을 대하실 때 늘 ' + workLine + '으로 살아오신 분이라, 이 시기에도 한가하게 흘려보내기보다는 “덜 무거운, 그러나 의미 있는 한 가지”에 손을 두십니다. '
        + tonePhrase(t9,
            '몸이 따라 주는 만큼, 후배·자녀·제자 한 명에게 ' + nmDnim(name) + '의 한 마디가 인생을 바꾸는 일이 일어납니다. 그 일이 평생 가장 큰 보람이 되어 줘요.',
            '몸의 신호가 본격적으로 시작되니, “무리한 일정보다 회복의 시간”이 가장 큰 자산이 됩니다. 작은 산책·일정한 식사·짧은 낮잠 같은 단순한 리듬이 ' + nmEulReul(name) + ' 가장 오래 가게 합니다.',
            '큰 행사 없이 일상의 깊이가 ' + nmUi(name) + ' 노년을 빛냅니다. 한 권의 책, 한 사람과의 산책, 한 끼의 정성스러운 식사 — 그런 작은 일들이 사실 평생의 결실입니다.');

    var s10 = '80~89세 — 외부 무대는 거의 정리되고, ' + nmDnim(name) + '의 진짜 본질이 가장 또렷이 드러나는 시기입니다. ' + (coreLine ? '“' + coreLine + '” — ' + nmDnim(name) + '을 한 줄로 정리하는 이 인상이 이때만큼 ' + nmEulReul(name) + ' 닮은 적이 없습니다. ' : nmDnim(name) + ' 안의 가장 깊은 본질이 풍경처럼 펼쳐지는 시기입니다. ') + '이 시기에 ' + nmEulReul(name) + ' 찾아오는 사람들은 더 이상 ' + nmUi(name) + ' 직책이나 성취가 아니라 ' + nmUi(name) + ' “사람 그 자체”를 보러 옵니다. '
        + tonePhrase(t10,
            '쌓은 것이 한 번 더 빛나는 흐름이라, 80대 중반에 가족 모임 한 번이 ' + nmUi(name) + ' 평생을 압축한 풍경처럼 펼쳐집니다.',
            '몸의 한계가 한 번씩 ' + nmEulReul(name) + ' 멈추게 하지만, 그 멈춤 속에서 ' + nmDnim(name) + '은 평생 처음으로 “아무것도 안 해도 된다”는 자유를 깊이 느끼시는 시기입니다.',
            '평온하게 흘러가는 시기지만, 이 시기에 가족·후배 한 명에게 ' + nmDnim(name) + '의 한 마디가 인생의 좌표가 됩니다. 그게 ' + nmUi(name) + ' 진짜 유산이에요.');

    var s11 = '90~99세 — 시간이 천천히 흐르고, 작은 일 하나하나가 큰 의미가 되는 시기입니다. 평생 ' + nmUi(name) + ' 마음을 풀어 놓으실 자리가 의외로 적었던 분이라, 이 시기에 와서야 비로소 “있는 그대로의 ' + nmDnim(name) + '”으로 살아 보시는 자유가 따라옵니다. 손에 잡히는 것들 — 책 한 권, 차 한 잔, 창밖의 햇살 — 그런 가벼운 것들의 무게가 가장 크게 다가오는 시기예요. '
        + tonePhrase(t11,
            '늦은 인정이 또 한 번 따라옵니다. 그게 사회적인 상이든, 가족이 전하는 한 마디든 — 평생 가장 진한 자국으로 남습니다.',
            '몸이 마음만큼 따라 주지 않는 시기지만, ' + nmDnim(name) + '의 마음 안쪽 풍경은 평생 가장 평화로워집니다.',
            '특별한 사건 없이 잔잔히 흘러가지만, 그 잔잔함이야말로 평생 ' + nmDnim(name) + '이 만들어 오신 “자기 자리”의 결정체입니다.');

    var s12 = '100~120세 — 마침표를 찍는 시기입니다. 마지막 풍경에 닿는 순간, ' + nmDnim(name) + '이 남기시는 것은 — ' + (careerRaw ? '평생 ' + careerRaw + ' 분야에서 다듬어 온 손길과 ' : '') + '“말이 아니라 남는 습관 한 줄”입니다. 큰 재산이나 화려한 직책보다, ' + nmUi(name) + ' 한 마디 한 마디가 누군가의 일생에 끼친 작은 영향들이 진짜 유산이 돼요. ' + nmEunNeun(name) + ' 평생 “' + (firstImpression || '진중한 분') + '”의 인상으로 사셨고, 그 인상이 마지막 순간에도 변하지 않은 채 그대로 남습니다. 시기에 따라 풍경은 바뀌고, 빛의 각도도 달라졌지만, ' + nmDnim(name) + ' 안의 본질은 첫 호흡과 마지막 호흡 사이에서 단 한 번도 흔들리지 않았어요. 그 본질대로 사셨을 때 ' + nmDnim(name) + '은 가장 자연스러우셨고, 그래서 가장 빛나셨습니다.';

    var stages = [
        { label: '0 ~ 7세 · 첫 풍경',          stripe: 'rgba(111,191,115,0.65)', body: s1 },
        { label: '8 ~ 13세 · 또래의 첫 만남',   stripe: 'rgba(111,191,115,0.55)', body: s2 },
        { label: '14 ~ 19세 · 자아의 첫 출발',   stripe: 'rgba(231,111,81,0.55)',  body: s3 },
        { label: '20 ~ 29세 · 사회로 나가는 첫 10년', stripe: 'rgba(231,111,81,0.65)', body: s4 },
        { label: '30 ~ 39세 · 자리잡기와 결혼',  stripe: 'rgba(212,175,55,0.65)',  body: s5 },
        { label: '40 ~ 49세 · 인생의 중간점과 재정의', stripe: 'rgba(212,175,55,0.55)', body: s6 },
        { label: '50 ~ 59세 · 책임의 무게와 후반전 출발', stripe: 'rgba(201,205,210,0.55)', body: s7 },
        { label: '60 ~ 69세 · 자유 회복의 시기', stripe: 'rgba(201,205,210,0.45)',  body: s8 },
        { label: '70 ~ 79세 · 본 모습이 가장 자연스러운 시기', stripe: 'rgba(120,140,180,0.55)', body: s9 },
        { label: '80 ~ 89세 · 본질이 또렷이 드러나는 시기', stripe: 'rgba(120,140,180,0.50)', body: s10 },
        { label: '90 ~ 99세 · 만년의 깊이',     stripe: 'rgba(157,140,180,0.55)',  body: s11 },
        { label: '100 ~ 120세 · 마지막 풍경 · 유산', stripe: 'rgba(199,167,106,0.65)', body: s12 }
    ];

    var stageHtml = stages.map(function (st) {
        return '<div class="life-stage-card sajux-print-surface" style="margin:0 0 18px;padding:22px 22px;border-radius:12px;border:1px solid rgba(199,167,106,0.22);background:rgba(199,167,106,0.04);border-left:3px solid ' + st.stripe + ';">'
            + '<div style="font-size:11px;letter-spacing:0.18em;color:var(--gold);font-weight:700;margin-bottom:14px;">' + escHtmlAttr(st.label) + '</div>'
            + '<p style="font-size:14px;color:var(--text);line-height:2.05;margin:0;">' + boldStarsToStrong(_vp(st.body)) + '</p>'
            + '</div>';
    }).join('');

    var introText = nmDnim(name) + '의 인생을 한 편의 이야기로 풀어 드릴게요. 태어남부터 마지막 장면까지 — 어떤 환경에서 자라셨고, 어떤 시기를 지나, 어디에 닿게 되는지를 시기별로 깊이 있게 살펴봅니다. 사주의 글자가 무엇인지는 잠시 잊으셔도 좋아요. 결과적으로 ' + nmIGa(name) + ' 어떤 사람으로 태어나, 어떻게 살아가시게 되는지를 그대로 읽으시면 됩니다.';

    var outroText = '여기까지가 ' + nmDnim(name) + '의 인생 한 편입니다. 시기는 흘러도 ' + nmUi(name) + ' 본질은 첫 호흡과 마지막 호흡 사이에서 변하지 않습니다. 무대가 화려할 때는 너무 멀리 가지 마시고, 무대가 어두울 때는 너무 깊이 가라앉지 마십시오. ' + nmUi(name) + ' 사주는 늘 그 사이의 길을 알려 주고 있어요.';
    ...
    return '<div id="sec-personal-portrait" class="report-chapter chapter-start sajux-panel-plain" style="margin:28px 0 40px;padding:28px 24px;border-radius:14px;border:1px solid rgba(199,167,106,0.30);background:transparent;">'
        ...
        + '<p style="font-size:12px;color:rgba(199,167,106,0.75);margin:0 0 22px;letter-spacing:0.04em;">120세 인생을 12개 시기로 — 한 사람의 이야기로 깊이 풀어 드립니다</p>'
        + '<p style="font-size:13.5px;color:var(--text);line-height:2;margin:0 0 22px;">' + boldStarsToStrong(_vp(introText)) + '</p>'
        + stageHtml
        + '<div class="life-stage-outro sajux-print-surface" style="margin:8px 0 0;padding:18px 20px;border-radius:12px;background:rgba(199,167,106,0.06);border-left:3px solid var(--gold);">'
        + '<p style="font-size:13.5px;color:var(--text);line-height:1.95;margin:0;">' + boldStarsToStrong(_vp(outroText)) + '</p>'
        + '</div>'
        + '</div>';
}
...
function buildDaeunTimeline(data) {
    ...
    return `<div class="daeun-timeline-wrap" style="margin-bottom:24px;">
        <div style="font-size:11px;color:rgba(255,255,255,0.45);letter-spacing:1px;margin-bottom:10px;">대운 흐름 — 10년마다 바뀌는 인생의 큰 계절 &nbsp;
...
function buildPartHeader(num, title, subtitle, anchorId, opts) {
    ...
    return '<div' + idAttr + ' class="part-header-block report-chapter sajux-print-surface" style="display:block;background:linear-gradient(135deg,rgba(' + color + ',0.09),rgba(255,255,255,0));border-top:2px solid ' + border + ';border-radius:16px;padding:28px 32px;margin:40px 0 4px;page-break-before:always;break-before:page;page-break-inside:avoid;break-inside:avoid;page-break-after:avoid;break-after:avoid;">'
        ...
        + '<div class="part-header-title" style="display:block;font-size:26px;font-weight:700;color:var(--text-primary);margin-bottom:8px;">' + icon + ' ' + title + '</div>'
...
function buildTOC(data) {
    ...
    var groups = [
        ...
        { head: '1부 — 나라는 사람', sub: '원국 · 기질 · 타고난 패턴', items: [
            { t: '일주 프로파일', s: '60일주 물상 · 키워드 · 신강신약', p: '—' },
            { t: '타고난 8자 · 인생 서사', s: '원국 분석 · 직업·애정 기질', p: '8p' },
            { t: '오행 지도 · 십성 패턴', s: '에너지 분포 · 업무 스타일', p: '7p' }
        ]},
        { head: '2부 — 지금 이 시절', sub: '대운 위치 · 올해 · 이달', items: [
            { t: '대운 타임라인', s: '생년~말년 10년 계절 지도', p: '—' },
            { t: '대운 80년 상세', s: '현재 대운 집중 해부', p: '10p' },
            { t: '올해 세운 · 월운 12개월', s: '연도 전술 · 월별 실행 지도', p: '10p' }
        ]},
        ...
        { head: '4부 — 지금부터의 선택', sub: '개운법 · 체크리스트 · 청사진', items: [
            { t: '[별첨] 자미두수 운명 설계도', s: '심리·재물·관계 축 근사 해석', p: '—' },
            { t: '개운법 · 최종 실행 지침', s: '영역별 체크리스트', p: '6p' }
        ]}
    ];
    var body = '';
