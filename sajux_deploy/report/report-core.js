...
/** 수 오행 한자용 인라인 스타일 — CSS --water 파란색, 윤곽 없음 */
...
function buildChapterHeadMainSub(mainTitle, subTitle, opts) {
    ...
    var julAttr = (opts.sajuxJul != null && String(opts.sajuxJul) !== '') ? ' data-sajux-jul="' + escHtmlAttr(String(opts.sajuxJul)) + '"' : '';
...
var SAJUX_SECTION_REGISTRY = {
    ...
    upcomingWolunA: { part: 2, section: 6, title: '앞으로 올 월운 · 전반', eyebrow: '다음 달부터 6개월', topic: 'monthly', inToc: true, headerMode: 'mainSub' },
...
var SAJUX_COMPAT_SECTION_REGISTRY = {
    ...
    friend: { part: 1, section: 7, title: '우정', eyebrow: '오래 가는 거리', topic: 'friend', inToc: true },
    business: { part: 1, section: 8, title: '동업 · 파트너', eyebrow: '역할과 돈', topic: 'business', inToc: true },
    spouse: { part: 1, section: 9, title: '배우자궁', eyebrow: '끌림의 방향', topic: 'spouse', inToc: true },
...
function buildCompatTopicMetaphor(topic, ctx) {
    ...
    var pools = {
        ...
        conflict: function () {
            return nA + '님과 ' + nB + '님, 다툼의 뿌리는 성격이 아니라 **말버릇과 타이밍**인 날이 많아요';
        },
        fortuneMix: function () {
            return '지금 이 계절과 앞으로 겹치는 계절이 말해 주는, 두 분만의 **관계 날씨**예요';
        },
...
function buildCompatBridge(topic, ctx) {
    ...
    var bridges = {
        ...
        conflict:    '부부·연인의 생활 숫자를 보았으니, 다툼이 커지기 전에 쓸 **대화 처방**을 바로 적어 드립니다.',
        fortuneMix:  '말과 역할을 맞추었으니, 이제 두 분의 사주가 **언제 같은 방향으로 불어 오는지** 흐름으로 정리해 보겠습니다.',
...
var _COMPAT_STEM_KR = {'甲':'갑','乙':'을','丙':'병','丁':'정','戊':'무','己':'기','庚':'경','辛':'신','壬':'임','癸':'계'};
var _COMPAT_BRANCH_KR = {'子':'자','丑':'축','寅':'인','卯':'묘','辰':'진','巳':'사','午':'오','未':'미','申':'신','酉':'유','戌':'술','亥':'해'};
var _COMPAT_OH_KR = {wood:'목', fire:'화', earth:'토', metal:'금', water:'수'};
var _COMPAT_STEM_OH = {'甲':'wood','乙':'wood','丙':'fire','丁':'fire','戊':'earth','己':'earth','庚':'metal','辛':'metal','壬':'water','癸':'water'};

function compatIljuMeetPhrase(saju, role) {
    saju = saju || {};
    var ds = saju.dayStem || '';
    var db = saju.dayBranch || '';
    var oh = _COMPAT_STEM_OH[ds] || 'earth';
    var ilju = (_COMPAT_STEM_KR[ds] || '') + (_COMPAT_BRANCH_KR[db] || '') + '일주';
    return ilju + '의 ' + (_COMPAT_OH_KR[oh] || '토') + ' 기운';
}

function compatMeetPairPhrase(ctx) {
    var a = ctx.a || {};
    var b = ctx.b || {};
...
function compatComputeChemistryScore(ctx) {
    ...
    var adj = 0;
    if (ctx.isHap) adj += 11;
    if (ctx.shengAB || ctx.shengBA) adj += 9;
    if (ctx.samhapScore > 0) adj += 6;
    if (ctx.isChung) adj -= 9;
    if (ctx.a && ctx.b && ctx.a.strength !== ctx.b.strength) adj += 7;
    else if (ctx.a && ctx.b && ctx.a.strength === ctx.b.strength && ctx.a.strength === '신강') adj -= 5;
    return Math.max(12, Math.min(98, Math.round(base * 0.52 + adj + 8)));
}

function buildCompatChemScoreBanner(ctx, caption) {
...
function buildCompatLoverPanelHtml(ctx) {
    ...
    var loveSit = meet + ', 연애 초반엔 **만남 횟수**보다 “다음에 언제 보는지”가 마음을 붙이는 속도를 정합니다. 둘 다 바쁜 주에는 연락이 끊기고, 그 빈칸이 불안으로 커지기 쉬워요.';
    var loveConf = ctx.shengAB
        ? escHtmlAttr(aName) + '님이 애정을 **행동**으로 보여 주고, ' + escHtmlAttr(bName) + '님은 **말과 분위기**로 받는 날이 많습니다. 한쪽은 “왜 말로 안 해?”라고, 다른 쪽은 “왜 이렇게 밀어붙여?”라고 느낄 수 있어요.'
        : ctx.shengBA
        ? escHtmlAttr(bName) + '님이 정서를 채워 주면 ' + escHtmlAttr(aName) + '님은 편안해지지만, 받는 쪽이 **당연함**으로 느껴지면 표현이 줄어듭니다.'
        : '비슷한 결이면 편하지만 **새 자극**이 없을 때 권태가 올 수 있고, 다른 결이면 끌리지만 **해석 차이**가 커질 수 있어요.';
    var loveRes = '**주 1회 “약속 리듬”**(요일·시간 고정)을 두세요. 데이트 비용보다 “지켜진 약속”이 애정 점수를 더 올립니다.';
    var marrySit = '동거·결혼 이야기가 오르면, ' + meet + ' **생활 숫자**(돈·집·휴식)가 로맨스보다 먼저 부딪히는 그림입니다. 설레는 마음만으로 계약을 미루면 나중에 비용이 커져요.';
    var marryConf = ctx.isChung
        ? '일지 충은 “사랑은 하는데 생활은 다르다”는 말이 자주 나오는 패턴입니다. 작은 습관(청소·지출·연락)에서 **승부가 갈리기** 쉽습니다.'
        : ctx.isHap
        ? '일지 합은 편안해서 **어려운 말을 미루는** 위험이 있습니다. 돈·미래는 “알아서 맞겠지”로 넘기다 어느 날 크게 부딪힐 수 있어요.'
        : '에너지가 비슷하면 결정 속도가 맞지 않을 때 **서로를 답답해하는** 날이 반복될 수 있어요.';
    var marryRes = '큰 결정은 **용신·대운이 겹치는 해**에 맞추되, 그 전에 “공동 가계·휴식·연락 규칙”을 한 장으로 적어 두십시오. **서면 한 줄**이 감정을 지켜 줍니다.';
    var scripts = [
        aName + '님: “지금 내가 서운한 건 ○○이야. 네 잘못이라기보다 내 마음이 그래. 오늘 저녁 20분만 들어줄래?”',
...
function buildCompatFriendPanelHtml(ctx) {
    ...
    var bName = compatNm(ctx, 'b');
    var meet = compatMeetPairPhrase(ctx);
    var sit = meet + ', 친구로 오래 가려면 **만남 횟수**보다 “같이 하는 일 한 가지”가 닻이 됩니다. 둘 다 바쁘면 연락이 끊기고, 그 빈칸이 서운함으로 쌓이기 쉬워요.';
    var conf = ctx.isChung
        ? '의견이 부딪히면 “친구인데 왜 이렇게 말이 세?”라는 서운함이 남습니다. **그날 밤 장문 메시지**는 비용이 큽니다.'
        : ctx.isHap
        ? '너무 편해서 **어려운 말을 미루는** 패턴이 생깁니다. 어느 날 한쪽만 서운함이 쌓여 “갑자기 멀어진” 느낌이 올 수 있어요.'
        : '관심사가 달라 **대화 주제가 끊기는** 날이 반복되면, 친함이 약해졌다고 느끼기 쉽습니다.';
    var res = '**한 달에 한 번, 같은 취미·같은 시간**만 고정하십시오. 싸운 날은 “내일 점심에 15분”만 약속해도 우정이 회복됩니다.';
    var sit2 = (ctx.samhapScore > 0)
        ? meet + ' **같은 방향 에너지**가 있어, 공동 목표(프로젝트·운동·공부)를 잡으면 우정이 깊어지기 쉽습니다.'
        : (ctx.score >= 60)
        ? escHtmlAttr(aName) + '님과 ' + escHtmlAttr(bName) + '님은 서로 다른 시각이 **영감**이 되는 장기 인연입니다.'
...
function buildCompatBizPanelHtml(ctx) {
    ...
    var b = ctx.b || {};
    var meet = compatMeetPairPhrase(ctx);
    var sit = meet + ', 동업·파트너십은 **친함**보다 “누가 무엇을 결정하는지”가 먼저입니다. 수익이 오르기 전에 역할이 겹치면, 사소한 지출 하나에도 신뢰가 갈리기 쉬워요.';
    var conf = ctx.shengAB
        ? escHtmlAttr(aName) + '님이 방향을 잡고 ' + escHtmlAttr(bName) + '님이 실행하는 구조가 맞지만, **“내가 다 기획했다”**는 말이 쌓이면 받는 쪽이 지칩니다.'
        : ctx.shengBA
        ? escHtmlAttr(bName) + '님이 전략을 세우고 ' + escHtmlAttr(aName) + '님이 밀어붙이는 그림이 좋지만, 속도 차이가 **책임 소재** 싸움으로 번지기 쉽습니다.'
        : '동등 파트너십이면 **결정권·돈·브랜드**가 한 사람에게 몰릴 때 비용이 가장 큽니다.';
    var res = '계약 전 **역할 한 줄**(누가 계약·누가 실행·누가 회계)을 서면으로 맞추십시오. 친구라도 **분기별 정산**은 필수입니다.';
    var sit2 = '회의에서 ' + escHtmlAttr(aName) + '님은 ' + (a.strength === '신강' ? '빠른 결론' : '신중한 검토') + '을, '
        + escHtmlAttr(bName) + '님은 ' + (b.strength === '신강' ? '큰 그림 제안' : '리스크 점검') + '을 선호하는 날이 많습니다.';
    var conf2 = a.strength === b.strength
        ? '둘 다 비슷한 속도라 합의는 빠르지만, **외부 검토 없이** 큰 판을 벌이다 손해 보기 쉽습니다.'
        : '속도가 다르면 빠른 쪽이 “왜 이렇게 느려?”라고, 신중한 쪽이 “왜 이렇게 무섭게 가?”라고 느낍니다.';
    var res2 = '중요 결정은 **48시간 유예** 후 서면 확정. 감정 회의 직후 계약서에 사인하지 마십시오.';
    var scripts = [
        aName + '님: “이번 프로젝트에서 내가 맡는 건 ○○까지야. 네 영역은 △△ — 이렇게 나누자.”',
...
function buildCompatMarriedWealthHtml(ctx) {
    ...
    var ac = a.counts || {};
    var bc = b.counts || {};
    var sit = meet + ', 부부로 살면 로맨스보다 **생활 숫자**(돈·집·휴식·가족 일정)가 먼저 맞물립니다. 둘 다 피곤한 달에 고정지출 이야기가 올라오면, 사랑 문제가 아니라 **숫자 문제**로 싸우기 쉬워요.';
    var conf = (ac.metal > 1 || ac.earth > 1) && !(bc.metal > 1 || bc.earth > 1)
        ? escHtmlAttr(aName) + '님은 쌓는 쪽, ' + escHtmlAttr(bName) + '님은 관계·정서를 먼저 보는 쪽이라 **지출 기준**이 갈리기 쉽습니다.'
...
function buildCompatMarriedPanelHtml(ctx, extraHtml) {
    var aName = compatNm(ctx, 'a');
    var bName = compatNm(ctx, 'b');
    var meet = compatMeetPairPhrase(ctx);
    var sc = ctx.score != null ? Number(ctx.score) : 55;
    var sit = meet + ', 부부 생활에서 **집안 일·육아·양가 방문**이 같은 주에 몰리면, 둘 다 지쳐서 말이 거칠어지기 쉽습니다. “사랑이 식었다”기보다 **체력이 비었다**는 신호인 날이 많아요.';
    var conf = ctx.isChung
        ? '생활 습관(청소·지출·연락)에서 **작은 것마다 각도**가 맞닿습니다. “그때는 괜찮았는데”가 쌓이면 큰 싸움이 됩니다.'
        : ctx.isHap
        ? '너무 편해서 **어려운 주제를 미루는** 패턴이 생깁니다. 돈·미래·자녀는 “알아서 맞겠지”로 넘기다 어느 날 폭발할 수 있어요.'
        : sc < 50
...
function buildCompatConflictPanelHtml(ctx) {
    var aName = compatNm(ctx, 'a');
    var bName = compatNm(ctx, 'b');
    var meet = compatMeetPairPhrase(ctx);
    var topic = ctx.isChung ? '속도·생활 습관' : ctx.shengAB || ctx.shengBA ? '주고받는 에너지' : '연락·돈·휴식';
    var sit = meet + ', 다툼이 커지기 전엔 보통 **' + topic + '**에서 작은 불씨가 납니다. 둘 다 “왜 이해 못 해?”라고 느끼는데, 사실은 **피곤함과 타이밍**인 날이 많아요.';
    var conf = ctx.isChung
        ? '같은 날 밤 긴 설명·장문 메시지는 **비용이 가장 큽니다**. 각자 방식을 고집할수록 “내가 맞다”가 앞섭니다.'
        : '한쪽이 먼저 사과하면 다른 쪽이 **더 세게** 말하는 패턴이 생기기 쉽습니다. 사과는 약함이 아니라 용기인데, 그걸 시험 삼는 순간이 위험합니다.';
    var res = '**하루 쿨다운 → 다음 날 점심 20분** 규칙을 두세요. 그 시간엔 “누가 옳은지” 금지, “나는 무엇이 불안했는지”만 말합니다.';
    var scripts = [
        aName + '님: “방금 말이 세게 나갔지? 네가 싫어서가 아니라, 나도 지쳐 있었어. 다시 말할게.”',
        bName + '님: “지금은 결론 내지 말자. 내일 아침에 커피 마시면서 20분만 이야기하자.”',
        aName + '님: “내가 원하는 건 이기는 게 아니라, 우리가 편해지는 거야. ○○만 같이 맞추자.”',
        bName + '님: “네 말 들었어. 나도 ○○ 때문에 속상했어. 오늘은 여기까지 하고 내일 이어가자.”'
    ];
...
function buildCompatFortuneMixPanelHtml(ctx) {
    ...
    var sc = ctx.score != null ? Number(ctx.score) : 55;
    var chem = compatComputeChemistryScore(ctx);
    var sit = meet + ', 지금 이 시기의 관계 날씨는 **케미스트리 ' + chem + '점**대로 읽힙니다. 궁합 총점(' + sc + '점)과 함께 보면, “잘 맞는가”보다 **언제 같이 움직이고 언제 쉬어야 하는가**가 분명해져요.';
    var conf = ctx.shengAB || ctx.shengBA
        ? '상생 결이 있어 **함께 시작**하기 좋은 시기가 오면, 한쪽만 앞서가면 다른 쪽이 지치기 쉽습니다.'
        : ctx.isChung
        ? '충 결은 **자극과 성장**이 같이 옵니다. 좋은 시기에도 말버릇을 조심하지 않으면 비용이 큽니다.'
        : '기복이 크지 않은 편이라, **작은 약속을 지키는 루틴**이 운을 올립니다.';
    var res = '아래 타임라인에서 **초록·별(✨) 카드**가 겹치는 해·구간에만 큰 결정(이사·동업·결혼)을 맞추십시오. **빨강·번개(⚡) 구간**엔 관계 점검만 하고 새 규칙은 만들지 마세요.';
...
function buildCompatPositioningPanelHtml(ctx) {
    ...
    var sit = meet + ', 관계에서 **누가 앞번호를 줄이는지**가 먼저 정해지지 않으면, 사랑은 있는데 피곔함만 쌓이는 그림이 자주 나옵니다. 바쁜 주·이사·돈 이야기가 겹치는 날, “서로를 몰라준다”는 느낌이 올라오기 쉬워요.';
    var gap = Math.abs(eA - eB);
    var conf = gap >= 16
        ? (eA > eB
            ? escHtmlAttr(aName) + '님 발이 더 빠른 날이 많아, ' + escHtmlAttr(bName) + '님이 **따라가느라 지치는** 패턴이 생기기 쉽습니다.'
            : escHtmlAttr(bName) + '님 발이 더 빠른 날이 많아, ' + escHtmlAttr(aName) + '님이 **밀려 난다**고 느끼기 쉽습니다.')
        : '속도는 비슷한데 **역할이 겹치면** 둘 다 “내가 더 했다”는 서운함이 동시에 올라옵니다.';
    var res = '**“이번 주는 내가 따른다”**를 번갈아 말하는 규칙 하나만 있어도, 포지션 싸움이 크게 줄어듭니다. 게이지 숫자는 힘 자랑이 아니라 **호흡 조율용 눈대중**입니다.';
    var leadSit = '큰 결정(이사·지출·일정) 앞에서, ' + meet + ' **누가 줄기를 잡고 누가 다듬을지**가 갈립니다.';
    var leadConf = (a.strength === '신강' && b.strength !== '신강')
        ? escHtmlAttr(aName) + '님이 방향을 잡으면 빠르지만, ' + escHtmlAttr(bName) + '님은 **말 없이 따라간다**고 느낄 수 있습니다.'
        : (b.strength === '신강' && a.strength !== '신강')
        ? escHtmlAttr(bName) + '님이 앞서가면 ' + escHtmlAttr(aName) + '님은 **존중받지 못한다**고 느낄 수 있습니다.'
        : '둘 다 주도형이면 **같은 날 같은 주제**를 두 번 논의하다 지치기 쉽습니다.';
    var leadRes = '큰 줄기는 **번갈아**, 세부는 **한 사람에게만** 묶고 같은 날 재논의하지 않는 규칙을 두십시오.';
    var coordMoney = ctx.shengAB || ctx.shengBA
        ? '**결론: 돈은 “벌면 지켜 줄 범위”**를 문장으로 먼저 정하십시오.'
        : '**결론: 돈은 통장 카테고리**만 같은 양식으로 나눠도 마찰이 줄어요.';
    var coordTalk = ctx.isChung
        ? '**결론: 충 결은 밤보다 낮 20분 카드**가 안전합니다.'
        : '**결론: 다툼 때 “내가 놓친 것 하나”**부터 말하십시오.';
    var coordRest = '**결론: 회복 시간을 달력에 겹쳐 박으십시오.**';
    var coordGoal = sc >= 60
...
function compatTimelineLine(cls, seed) {
    var pools = {
        good: [
            '두 분의 대운 기운이 같은 방향으로 흐르는 시기예요. 한쪽만 앞서 나가면 균형이 어긋날 수 있으니, 이번 달 안에 함께하고 싶은 일을 세 줄로 적어 역할을 나눠 시작해 보십시오. 이동·계약·프로젝트처럼 겉으로 보이는 결정도, 둘이 같은 날 실행하는 쪽으로 맞추면 힘이 배가됩니다.',
            '외부에서 들어오는 기회가 두 분에게 거의 동시에 닿는 해로 읽혀요. 혼자 잡으려다 놓치기 쉬우니, 주말에 기회 목록을 공유하고 우선순위 하나만 골라 같이 밀어 보세요. 한 사람은 실행, 다른 한 사람은 일정·돈·연락을 맡는 식으로 나누면 부담이 줄고 성과는 더 커집니다.',
            '이 십 년 구간에서 내린 약속이 다음 대운의 출발점이 됩니다. 결혼·동거·동업처럼 관계의 방향을 바꾸는 선택은 서두르지 말고, 서로에게 꼭 필요한 조건 세 가지를 말로 확인한 뒤 일정표에 박아 두십시오. 그 한 장이 흔들릴 때 다시 꺼내 보면 길이 보입니다.'
        ],
        hard: [
            '두 분의 대운 각도가 맞지 않아 작은 말도 크게 들릴 수 있는 시기예요. 감정이 올라온 날에는 결론을 내지 말고, 하루만 쿨다운 시간을 약속하세요. 그다음 날 짧게 “어제 내가 예민했어” 한마디만 해도 관계의 온도가 많이 내려가고, 같은 실수를 반복하지 않게 됩니다.',
            '한쪽은 숨이 차 오르는 해, 다른 한쪽은 회복이 필요한 해로 겹칠 수 있어요. “왜 나를 안 챙기냐”보다 “이번 달엔 회복 주간이 필요해”라고 먼저 말해 주세요. 일정·지출·휴식을 달력에 표시해 두면 서로의 템포 차이를 덜 오해하고, 비난 대신 배려가 먼저 나옵니다.',
            '바깥 일·가족·돈 문제가 관계 안으로 스며들기 쉬운 구간입니다. 이사·투자·큰 지출·인사 이동처럼 되돌리기 어려운 결정은 최소 한 달 미루고, 대신 주간 점검 시간 30분만 고정하세요. 그 시간엔 해결책보다 서로의 말을 끝까지 듣는 것에만 쓰시면 됩니다.'
        ],
        half: [
            '크게 치고 올라가기보다 일상의 리듬을 다듬기 좋은 흐름이에요. 매주 같은 요일·같은 시간에 만나거나, 한 가지 공통 루틴만 정해 두세요. 화려한 이벤트보다 반복된 약속이 이 시기엔 관계를 더 단단하게 잡아 주고, 다음 좋은 대운 싱크 때 힘을 실어 줍니다.',
            '사건운이 강하지 않아 조용히 지나가는 해로 읽혀요. 그래서 오히려 대화·식사·산책 같은 작은 접점을 의식적으로 늘리면 좋습니다. 한 달에 한 번, “요즘 우리 관계에서 좋았던 점 하나”만 서로 말하는 습관을 시작해 보세요. 짧아도 기록이 쌓이면 안정감이 커집니다.',
            '관계 점검과 목표 재확인에 알맞은 시기입니다. 각자 바라는 생활 그림을 한 문장으로 적어 바꿔 읽어 보세요. 맞는 부분은 올해 계획에 넣고, 다른 부분은 타협이 아니라 조율 일정으로 잡으면 다음 좋은 대운 싱크 때 훨씬 수월하게 큰 결정을 맞출 수 있습니다.'
        ]
    };
...
function buildTopicOpenerInner(topic, data) {
    ...
    var pools = {
        ...
        sipseong: [
            nmDnim(nm) + '에게 가장 잘 맞는 무기는 아래 세 단락으로만 짚었어요.',
            nmDnim(nm) + '의 **' + sipWeaponLabelForGroup(sipToSipGroupKey(mainSip) || '재성', mainSip) + '**이 일과 관계에서 먼저 나오는 타입이에요.'
        ],
        wealth: [
            '쌓는 해와 지키는 해가 갈립니다.',
...
function colorizeHanjaInline(html) {
    ...
    var COLOR = { wood:'var(--wood, #6FBF73)', fire:'var(--fire, #E76F51)', earth:'var(--earth, #C8A45C)', metal:'var(--metal, #C9CDD2)', water:'var(--water, #1a6fd4)' };
    ...
    s = s.replace(HAN_RE, function (ch) {
        ...
        if (!oh) return ch;
        var col = COLOR[oh];
        return '<span class="hanja-oh h' + oh + '" style="color:' + col + ';font-family:\'Noto Sans KR\',sans-serif;font-weight:500">' + ch + '</span>';
    });
...
/** 3단락 풀이용 — 그룹별 무기 1개·현장 예시·루틴 (키워드 다이어트) */
var SAJUX_SIP_WEAPON_SCENE = {
    비겁: {
        shine: '혼자서도 방향을 잡고, 실력·지분이 걸린 자리에서 체감이 큽니다.',
        example: '동업 조건을 먼저 문장으로 정리하거나, 본인 역할을 회의 전에 한 줄로 박아 둘 때',
        routine: '이번 달 **역할 한 줄**을 메모해 두고, 중요한 회의·협상에 들어가 보십시오.'
    },
    식상: {
        shine: '기획서·슬라이드·설계도처럼 손으로 만든 결과물이 곧 평가로 이어집니다.',
        example: '발표 자료를 직접 다듬거나, 하루 만에 시안·프로토타입을 보여 줄 때',
        routine: '이번 달 **완성본 한 장**(기획서·포트폴리오·시안)을 먼저 만들고, 그다음에 설명하십시오.'
    },
    재성: {
        shine: '견적·정산·거래처 관리처럼 숫자와 현실 감각이 앞서 나갑니다.',
        example: '견적서를 치밀하게 쓰거나, 월 지출·입금을 표 한 장으로 맞출 때',
        routine: '매주 금요일 20분, **입금·지출 표**만 열어 보는 루틴을 한 달 고정하십시오.'
    },
    관성: {
        shine: '마감·책임·조직 안에서 이름이 박히는 자리에서 힘이 납니다.',
        example: '프로젝트 마감을 맡거나, 팀 규칙·약속을 문서로 남길 때',
        routine: '중요한 약속은 **문장으로만** 남기고, 구두 합의는 다음 날 아침에 정리하십시오.'
    },
    인성: {
        shine: '자료 정리·자격·멘토링처럼 배움으로 버티는 쪽에서 오래 갑니다.',
        example: '시험·자격 공부를 끊기지 않게 하거나, 선배 한 명에게만 조언을 구할 때',
        routine: '이번 달 **45분 공부 블록**을 주 2회만 달력에 박아 두십시오.'
    }
};

var SAJUX_SIP_SYNERGY_LINE = {
    '비겁|식상': '주도권으로 방향을 잡고, 손끝 결과물로 증명하는 조합',
    '비겁|재성': '내 방식으로 벌고, 숫자로 마무리하는 조합',
    '비겁|관성': '앞장서서 밀고, 규칙으로 끝맺는 조합',
    '비겁|인성': '혼자 결정은 빠르고, 자료로 뒷받침하는 조합',
    '식상|재성': '기획·작품으로 그리고, 견적·계약으로 받는 조합',
    '식상|관성': '만든 것을 내놓고, 직함·평가로 인정받는 조합',
    '식상|인성': '깊이 파고, 자격·문서로 보완하는 조합',
    '재성|관성': '숫자로 판단하고, 책임으로 실행하는 조합',
    '재성|인성': '현실 감각과 학습이 번갈아 받쳐 주는 조합',
...
function sipWeaponLabelForGroup(groupKey, primarySip) {
    if (primarySip && SAJUX_SIP_MODERN.individual[primarySip]) {
        ...
        if (gk === groupKey) return SAJUX_SIP_MODERN.individual[primarySip];
    }
    var g = SAJUX_SIP_MODERN.group[groupKey];
    return (g && g.keywords && g.keywords[0]) ? g.keywords[0] : sipGroupBarLabel(groupKey);
}

function sipSynergyPhrase(topKey, secKey) {
    var a = topKey < secKey ? topKey + '|' + secKey : secKey + '|' + topKey;
    if (SAJUX_SIP_SYNERGY_LINE[a]) return SAJUX_SIP_SYNERGY_LINE[a];
    var w1 = sipWeaponLabelForGroup(topKey, '');
    var w2 = sipWeaponLabelForGroup(secKey, '');
    return w1 + '으로 밀고 ' + w2 + '으로 마무리하는 흐름';
}

/** 십성 통합 풀이 — 메타·원리 설명 제거 */
function stripSipseongMetaNarrative(s) {
    ...
    var patterns = [
        ...
        /시험·자격·서류[^.!?]*앞에서는[^.!?]*[.!?]/g,
        /동료·동업[^.!?]*앞에서는[^.!?]*[.!?]/g,
        /「[^」]+」\s*쪽이[^.!?]*먼저\s*움직이는\s*날이\s*많습니다[^.!?]*[.!?]/g,
        /이름을\s*하나하나\s*외우[^.!?]*[.!?]/g,
        /언제\s*어떤\s*반응이\s*자동으로[^.!?]*[.!?]/g
    ];
...
function buildChapter3SipseongThreeParagraphNarrative(data, topG, secondG, primaryList) {
    ...
    var secKey = (secondG && secondG.sum > 0 && secondG.key !== topKey) ? secondG.key : null;
    var topSip = (primaryList && primaryList[0]) ? primaryList[0] : '';
    var secSip = '';
    if (secKey && primaryList && primaryList.length) {
        for (var i = 0; i < primaryList.length; i++) {
            if (sipToSipGroupKey(primaryList[i]) === secKey) {
                secSip = primaryList[i];
                break;
            }
        }
    }
...
SAJUX_VOICE.fortuneNarrativePrompt = '';
...
function buildGongmangDeepHookCopy(data) {
    ...
    var tagByN = {'년주': '년지(가문·출발)', '월주': '월지(부모·직업)', '일주': '일지(배우자·자아)', '시주': '시지(자녀·황혼)'};
    ...
    var resultByPillar = {
        ...
        '시주': '**시지** — 자녀·황혼·성과가 겉과 속이 다르게 읽히고, 끝맺음이 애매하게 남을 수 있습니다.'
    };
    var lines = [];
...
function ensureHtml2CanvasLoaded(done) {
    ensureExternalScriptLoaded({
        ...
        src: getReportAssetUrl('vendor/html2canvas.min.js'),
        ready: function () { return typeof html2canvas === 'function'; },
        done: done,
        fail: function () {
            ensureExternalScriptLoaded({
                id: 'sajux-html2canvas-script-cdn',
                ...
                fail: function () {
                    ...
                    alert('사주 저장 도구(html2canvas)를 불러오지 못했습니다.\nWi-Fi 확인 후 새로고침해 주세요.');
                }
            });
        }
    });
...
function ensureJsZipLoaded(done) {
    ensureExternalScriptLoaded({
        ...
        src: getReportAssetUrl('vendor/jszip.min.js'),
        ready: function () { return typeof JSZip === 'function'; },
        done: done,
        fail: function () {
            ensureExternalScriptLoaded({
                id: 'sajux-jszip-script-cdn',
                ...
                fail: function () {
                    sajuxReleaseCaptureBusy();
                    sajuxHideCaptureOverlay();
                    alert('사주 저장 도구(JSZip)를 불러오지 못했습니다.\nWi-Fi 확인 후 새로고침해 주세요.');
                }
            });
        }
    });
...
function ensureHtmlToImageLoaded(done) {
    ensureExternalScriptLoaded({
        id: 'sajux-html-to-image-script',
        src: 'https://cdn.jsdelivr.net/npm/html-to-image@1.11.11/dist/html-to-image.js',
        ready: function () { return typeof htmlToImage !== 'undefined' && typeof htmlToImage.toCanvas === 'function'; },
        done: done,
...
function sajuxCanvasFromLiteCard(div, w) {
    w = w || Math.min(720, window.innerWidth || 390);
    var jul = div.children[0] ? (div.children[0].textContent || '') : '';
    var title = div.children[1] ? (div.children[1].textContent || '') : '';
    ...
    ctx.fillStyle = '#c7a76a';
    ctx.font = '600 13px "Apple SD Gothic Neo","Noto Sans KR",sans-serif';
    ctx.fillText(jul, 18, 28);
    ctx.fillStyle = '#f5f0e6';
    ctx.font = '700 17px "Apple SD Gothic Neo","Noto Sans KR",sans-serif';
    ctx.fillText(title.slice(0, 48), 18, 52);
    ctx.fillStyle = '#cccccc';
    ctx.font = '14px "Apple SD Gothic Neo","Noto Sans KR",sans-serif';
    lines.forEach(function (line, i) {
        ctx.fillText(line, 18, 78 + i * 17);
    });
    return sajuxCanvasToBlob(canvas, 'image/png');
}
function sajuxBuildMobileLiteCard(jul, title, bodyText) {
    var div = document.createElement('div');
    div.className = 'sajux-mobile-lite-card';
    div.style.cssText = 'background:#050508;padding:22px 18px;box-sizing:border-box;width:100%;max-width:720px;color:#e8e8e8;font-family:"Apple SD Gothic Neo","Noto Sans KR",sans-serif;';
    var hJul = document.createElement('div');
    hJul.style.cssText = 'color:#c7a76a;font-size:13px;margin-bottom:6px;';
    hJul.textContent = jul || '';
    var hTitle = document.createElement('div');
    hTitle.style.cssText = 'color:#f5f0e6;font-size:17px;font-weight:700;margin-bottom:14px;line-height:1.35;';
...
function sajuxConvertToMobileLiteCard(sourceEl, jul, title) {
    if (jul === '표지') {
        return sajuxBuildMobileLiteCard('표지', 'SAJUX', '사주X 프리미엄 리포트\n표지·로고는 웹 화면과 동일하게 보실 수 있습니다.');
    }
    var raw = '';
    if (sourceEl) {
        try { raw = sourceEl.innerText || sourceEl.textContent || ''; } catch (eTxt) { raw = ''; }
    }
    if (jul === '고객표지' && sourceEl) {
        var nm = sourceEl.querySelector('.cover-brush-name');
        var birth = sourceEl.querySelector('.birth-info');
        raw = (nm ? nm.textContent + '\n' : '') + (birth ? birth.textContent : '');
    }
...
function sajuxCaptureDestLabel(mode) {
    ...
    if (mode === 'share') return '저장 위치 · 다른 앱';
    if (mode === 'files') return '저장 위치 · 파일 앱';
    return '저장 위치 · 다운로드 폴더';
}
function sajuxEnsureCaptureOverlayVisible(el) {
    if (!el) return;
    el.style.cssText = 'position:fixed;inset:0;z-index:2147483647;background:rgba(0,0,0,0.82);display:flex;align-items:flex-start;justify-content:center;padding:0;box-sizing:border-box;visibility:visible;opacity:1;pointer-events:auto;overflow-y:auto;-webkit-overflow-scrolling:touch;';
}
function sajuxStartCaptureProgress(total, destMode) {
    ...
    el.innerHTML = ''
        ...
        + '<div class="sajux-cap-title-pulse" style="font-size:15px;font-weight:600;">사주 저장 중</div>'
        ...
        + (sajuxIsMobileDevice() ? '<div style="font-size:11px;color:rgba(245,240,232,.38);margin-top:8px;">총 ' + total + '장 · 장마다 10~20초 걸릴 수 있어요</div>' : '')
        + '</div>';
...
function sajuxShowCaptureOverlay(message, opts) {
    ...
    if (!el) {
        el = document.createElement('div');
        el.id = id;
        el.innerHTML = '<div class="sajux-capture-overlay-inner" style="width:min(320px,92vw);text-align:center;font-family:\'Noto Sans KR\',sans-serif;color:#f5f0e6;font-size:14px;line-height:1.75;white-space:pre-line;margin:auto;padding:32px 20px;"></div>';
        document.body.appendChild(el);
    }
...
var _sajuxPendingZip = null;
function sajuxClearPendingZip() {
    if (_sajuxPendingZip && _sajuxPendingZip.url) {
        try { URL.revokeObjectURL(_sajuxPendingZip.url); } catch (eRev) {}
    }
    _sajuxPendingZip = null;
}
function sajuxShareZipFile(zipBlob, filename, onDone) {
...
function sajuxClearDisplaySectionCache() {
    _sajuxDisplaySecCache.main = null;
    _sajuxDisplaySecCache.compat = null;
}
function sajuxIsMobileDevice() {
    try {
...
var _sajuxDesktopCaptureActive = false;
var SAJUX_CAPTURE_PAGE_W = 720;
var SAJUX_CAPTURE_WINDOW_W = 1280;
var SAJUX_CAPTURE_WINDOW_H = 900;
function sajuxBeginDesktopCaptureSession() {
    _sajuxDesktopCaptureActive = true;
    try { document.documentElement.classList.add('sajux-capture-desktop'); } catch (eCls) {}
}
function sajuxEndDesktopCaptureSession() {
    _sajuxDesktopCaptureActive = false;
...
function sajuxIsCaptureableEl(el) {
    ...
    return (el.offsetHeight > 4 || el.scrollHeight > 4 || el.getBoundingClientRect().height > 4);
}
...
function sajuxHeadIsCaptureSkipped(headEl) {
    if (!headEl || headEl.nodeType !== 1) return true;
    if (headEl.getAttribute('data-sajux-capture-skip') === '1') return true;
    if (headEl.closest('.sajux-no-capture, #sec-report-footer-utilities, .report-footer-utilities')) return true;
...
function sajuxFilterCaptureHeads(container, heads) {
    return (heads || []).filter(function (h) {
        return !sajuxHeadIsCaptureSkipped(h);
    });
}
function sajuxResolveCaptureEnd(container, startHead, endHead) {
    if (!endHead || !startHead) return endHead;
    var boundary = container ? container.querySelector('[data-sajux-capture-boundary="part4"]') : null;
    if (!boundary) return endHead;
    var startJul = startHead.getAttribute('data-sajux-jul') || '';
    if (!/^4-/.test(startJul)) return endHead;
    var endTitle = endHead.getAttribute('data-sajux-jul-title') || '';
    var endJul = endHead.getAttribute('data-sajux-jul') || '';
    if (endJul === '별첨' || endTitle.indexOf('자미두수') >= 0) return endHead;
    if (sajuxHeadIsCaptureSkipped(endHead) || endHead.closest('.sajux-no-capture, #sec-report-footer-utilities')) return boundary;
    return endHead;
}
...
function sajuxGroupMobileCompatCaptureSlices(slices) {
    if (!slices || !slices.length || !sajuxIsMobileDevice()) return slices || [];
    var cover = [], summary = [], blockA = [], blockB = [], blockC = [];
    function julNum(s) { var m = String(s.jul || '').match(/^1-(\d+)/); return m ? parseInt(m[1], 10) : 99; }
    for (var i = 0; i < slices.length; i++) {
        var jul = String(slices[i].jul || '');
        if (jul === '표지') cover.push(slices[i]);
        else if (jul === '요약') summary.push(slices[i]);
        else { var n = julNum(slices[i]); (n <= 4 ? blockA : n <= 8 ? blockB : blockC).push(slices[i]); }
    }
    var plan = [
...
function sajuxMergeCaptureSlices(slices) {
    ...
    for (var i = 0; i < slices.length; i++) {
        ...
        var julA = String(cur.jul || '');
        var julB = String(nxt.jul || '');
        var titleA = String(cur.title || '');
        var titleB = String(nxt.title || '');
        var mergePalace = (titleA.indexOf('중반') >= 0 || titleA.indexOf('중·후반') >= 0) && titleB.indexOf('후반') >= 0;
        var mergeLegal = julA === '5-20' && julB === '5-21';
        var mergeLegalAlt = titleA.indexOf('개인정보') >= 0 && titleB.indexOf('이용약관') >= 0;
        if (mergePalace || mergeLegal || mergeLegalAlt) {
            out.push(sajuxMergeCaptureSlicePair(cur, nxt));
            i += 1;
            continue;
        }
        out.push(cur);
...
function sajuxCollectJulHeads(container) {
    ...
    container.querySelectorAll('.ch-head-main-sub[data-sajux-jul], .ch-head-topic-first[data-sajux-jul], .ziwei-surprise-intro[data-sajux-jul], .report-chapter.sajux-capture-chapter > .ch-head-main-sub[data-sajux-jul]').forEach(function (h) {
        ...
        if (sajuxHeadIsCaptureSkipped(h)) return;
        seen[k] = true;
        heads.push(h);
    });
    if (!heads.length) {
        container.querySelectorAll('.ch-head-main-sub, .ch-head-topic-first').forEach(function (h) {
            if (!sajuxIsVisibleEl(h) || !h.querySelector('.ch-sec-num')) return;
            heads.push(h);
        });
    }
    return heads;
}
function sajuxPushCaptureSlice(slices, container, startNode, endNode, headEl) {
    var jul = headEl.getAttribute('data-sajux-jul') || '';
    var title = headEl.getAttribute('data-sajux-jul-title') || '';
    if (!jul) {
        var numEl = headEl.querySelector('.ch-sec-num');
        if (numEl) jul = (numEl.textContent || '').trim();
    }
    if (!title) {
        var titEl = headEl.querySelector('.ch-sec-title');
    ...
    var rangeHeads = [];
    var inRange = false;
    var allHeads = sajuxCollectJulHeads(container);
    for (var hi = 0; hi < allHeads.length; hi++) {
        if (allHeads[hi] === startNode) inRange = true;
        if (!inRange) continue;
        if (endNode && allHeads[hi] === endNode) break;
        rangeHeads.push(allHeads[hi]);
    }
    if (rangeHeads.length <= 1) {
        slices.push({ el: wrap, jul: jul, title: title, headEl: headEl });
        return;
    }
    for (var si = 0; si < rangeHeads.length; si++) {
        var h0 = rangeHeads[si];
        var h1 = rangeHeads[si + 1] || endNode;
        var j2 = h0.getAttribute('data-sajux-jul') || jul;
        var t2 = h0.getAttribute('data-sajux-jul-title') || title;
        var subWrap = sajuxWrapDomRange(container, h0, h1, { jul: j2, title: t2 });
...
function sajuxCollectCompatCaptureSlices(root) {
    ...
    var titleEl = container.querySelector('#sec-compat-title');
    if (coverEl && titleEl && sajuxIsVisibleEl(coverEl) && sajuxIsVisibleEl(titleEl)) {
        var coverWrap = document.createElement('div');
        coverWrap.className = 'sajux-capture-jul-wrap';
        coverWrap.style.cssText = 'background:#050508;box-sizing:border-box;width:100%;padding:0;margin:0;';
        coverWrap.setAttribute('data-sajux-jul', '표지');
        coverWrap.setAttribute('data-sajux-jul-title', '표지');
        coverWrap.appendChild(coverEl.cloneNode(true));
        coverWrap.appendChild(titleEl.cloneNode(true));
        slices.push({ el: coverWrap, jul: '표지', title: '표지' });
    } else {
        sajuxCompatPushCloneSlice(slices, coverEl, '표지', '브랜드');
        sajuxCompatPushCloneSlice(slices, titleEl, '표지', '궁합 표지');
    }
    ...
    var sectionIds = [
        ...
        'section-conflict', 'section-friend', 'section-business', 'section-spouse',
        'section-ideal-family', 'section-fortune-mix', 'section-timeline'
    ];
    ...
    sectionIds.forEach(function (sid) {
        ...
        if (topicKey === 'fortune-mix') topicKey = 'fortuneMix';
...
function sajuxCollectCaptureSlices(root) {
    ...
    for (var i = 0; i < heads.length; i++) {
        var endNode = sajuxResolveCaptureEnd(container, heads[i], heads[i + 1] || null);
        sajuxPushCaptureSlice(slices, container, heads[i], endNode, heads[i]);
    }
    ...
    tailDefs.forEach(function (def) {
        ...
        if (el.querySelector && el.querySelector('.ch-head-main-sub[data-sajux-jul], .ch-head-topic-first[data-sajux-jul]')) return;
        if (tailSeen[el.id || def.sel]) return;
...
function sajuxValidateCaptureCanvas(canvas) {
    ...
    try {
        var ctx = canvas.getContext('2d');
        if (!ctx) return true;
        var step = Math.max(4, Math.floor(Math.min(canvas.width, canvas.height) / 24));
        var found = false;
        for (var y = 0; y < canvas.height && !found; y += step) {
            for (var x = 0; x < canvas.width; x += step) {
                var a = ctx.getImageData(x, y, 1, 1).data[3];
                if (a > 8) { found = true; break; }
            }
        }
        return found;
    } catch (eVal) {
        return canvas.width > 12 && canvas.height > 12;
    }
}
function sajuxValidateCaptureBlob(blob) {
    return !!(blob && blob.size > 256);
}
function sajuxHtmlToImageBlobPromise(target, scale, timeoutMs) {
    return sajuxEnsureHtmlToImagePromise().then(function () {
        return new Promise(function (resolve, reject) {
            ...
            htmlToImage.toBlob(target, {
                backgroundColor: '#050508',
                pixelRatio: Math.max(1, Math.min(scale || 1, 2)),
                cacheBust: true,
                skipFonts: false
            }).then(function (blob) {
                if (done) return;
                ...
                clearTimeout(timer);
                if (sajuxValidateCaptureBlob(blob)) resolve(blob);
...
function sajuxCaptureSliceToBlob(target, capOpts, timeoutMs) {
    var mobile = sajuxIsMobileCapture();
    var scales = [capOpts.scale];
    if (capOpts.scale > 1) scales.push(Math.max(0.85, capOpts.scale * 0.72));
    scales.push(1);
    function h2cAttempt(i) {
        var sc = scales[i];
        var opts = {};
        for (var k in capOpts) { if (Object.prototype.hasOwnProperty.call(capOpts, k)) opts[k] = capOpts[k]; }
        opts.scale = sc;
        opts.foreignObjectRendering = false;
        return sajuxHtml2canvasPromise(target, opts, timeoutMs).then(function (canvas) {
            if (sajuxValidateCaptureCanvas(canvas)) return sajuxCanvasToBlob(canvas, 'image/png');
            if (i + 1 >= scales.length) return Promise.reject(new Error('blank canvas'));
            return h2cAttempt(i + 1);
        });
    }
    function h2iAttempt(i) {
        var sc = scales[i] || 1;
        return sajuxHtmlToImageBlobPromise(target, sc, timeoutMs).catch(function (err) {
            if (i + 1 >= scales.length) return Promise.reject(err);
            return h2iAttempt(i + 1);
        });
    }
    if (mobile && typeof htmlToImage !== 'undefined' && typeof htmlToImage.toBlob === 'function') {
        return h2iAttempt(0).catch(function () { return h2cAttempt(0); });
    }
    return h2cAttempt(0);
}
...
function sajuxSlugCaptureLabel(raw) {
    var s = String(raw || '').replace(/[\\/:*?"<>|]/g, '').replace(/\s+/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '');
    if (!s) return 'section';
...
function sajuxCanvasToBlob(canvas, type) {
    var mime = type || sajuxCaptureImageMime();
...
function sajuxResolveCaptureTarget(slice, pack, captureHost) {
    ...
    var node = source ? source.cloneNode(true) : null;
    if (captureHost && node) {
        sajuxStyleCaptureHost(captureHost, true);
        captureHost.innerHTML = '';
        captureHost.appendChild(node);
        var via = captureHost.firstChild || node;
        via.style.width = '100%';
        via.style.maxWidth = sajuxCaptureHostWidthPx() + 'px';
        via.style.boxSizing = 'border-box';
        via.style.background = '#050508';
        sajuxPrepareSliceForCapture(via);
        return { target: via, viaHost: true };
    }
    if (node) {
        sajuxPrepareSliceForCapture(node);
        return { target: node, viaHost: false };
    }
...
function sajuxWaitCaptureLayout() {
    return new Promise(function (resolve) {
        var n = 0;
        var max = sajuxIsMobileCapture() ? 6 : 2;
        function step() {
            n += 1;
            if (n >= max) resolve();
...
function sajuxOnCaptureClone(doc) {
    ...
    var cloneRoot = doc.getElementById('report-container') || doc.getElementById('sec-report-full') || doc.getElementById('main-content');
    if (cloneRoot) {
        cloneRoot.style.overflow = 'visible';
        cloneRoot.style.maxHeight = 'none';
        cloneRoot.style.height = 'auto';
        cloneRoot.style.background = '#050508';
    }
}
...
function sajuxCanUseSaveFilePicker() {
    return typeof window.showSaveFilePicker === 'function';
}
function sajuxCanShareZipBlob(zipBlob, filename) {
    if (typeof navigator === 'undefined' || typeof navigator.share !== 'function') return false;
    try {
        var file = sajuxMakeZipShareFile(zipBlob, filename);
...
function sajuxOfferZipDownload(zipBlob, filename, extraMsg, captures) {
    ...
    var canPick = sajuxCanUseSaveFilePicker();
    var dest = window.__sajuxCaptureDest || '';
    var msg = '✅ ZIP 준비 완료!\n\n' + (mobileDevice ? sajuxMobileSaveHint() : '「ZIP 저장」 또는 「저장 위치 선택」을 눌러 주세요.');
    msg += (extraMsg || '');
    var buttons = [];
    function showSavedNote(note) {
        sajuxShowCaptureOverlay(note || '✅ 저장 완료!', {
            buttons: [{ label: '닫기', primary: false, onClick: function () {
                sajuxHideCaptureOverlay();
                sajuxClearPendingZip();
            } }]
        });
    ...
    sajuxShowCaptureOverlay(msg, { buttons: buttons });
    if (!mobileDevice && dest !== 'picker') {
        setTimeout(function () {
            if (_sajuxPendingZip && _sajuxPendingZip.url) {
                sajuxDownloadBlob(_sajuxPendingZip.url, _sajuxPendingZip.filename);
            }
        }, 120);
    }
}
...
function sajuxBuildCaptureZipBlob(captures, baseName) {
    ...
    captures.forEach(function (item) {
        var ext = item.ext || sajuxCaptureImageExt();
        folder.file(item.name + ext, item.blob);
    });
...
function sajuxBuildCaptureBaseName(data) {
    ...
    return '사주X-' + safe + '-' + new Date().toISOString().slice(0, 10);
}
...
/* ── 모바일 라이브 DOM 크롭 캡처 ─────────────────────────────────────
   ...
   → 모바일 레이아웃 그대로, 빈공간 없음, PC 레이아웃 강제 없음. */
function sajuxMobileGetLiveGroups(container) {
    var heads = sajuxFilterCaptureHeads(container, sajuxCollectJulHeads(container));
    function classify(jul) {
        if (jul === '표지') return 0;
        if (jul === '목차') return -1;
        if (jul === '보너스' || jul === '별첨') return 7;
        var m = jul.match(/^(\d+)-(\d+)/);
        if (!m) return 7;
        var part = parseInt(m[1], 10), sec = parseInt(m[2], 10);
        if (part === 1) return sec === 1 ? 0 : 1;
        if (part === 2) { if (sec <= 3) return 2; if (sec === 4) return 3; return 4; }
        if (part === 3) return sec <= 2 ? 5 : 6;
        return 7;
    }
    var titles = ['표지·일주', '1부 원국', '현재 운세', '앞으로 올 대운', '세운·월운', '애정·재물', '합격·직업·건강', '개운법·마무리'];
    var firstEl = new Array(8).fill(null);
    /* 표지: 컨테이너 최상단(Y=0) */
    firstEl[0] = container.querySelector('#sec-cover, #sec-client-cover, .cover-page') || container.firstElementChild;
    heads.forEach(function(h) {
        var idx = classify(h.getAttribute('data-sajux-jul') || '');
        if (idx >= 0 && !firstEl[idx]) firstEl[idx] = h;
    });
    var groups = [];
    for (var i = 0; i < 8; i++) {
        if (firstEl[i]) groups.push({ idx: i, title: titles[i], el: firstEl[i] });
    }
    return groups;
}
...
function sajuxRunMobileLiveCrop(root, fab, restorePattern, starStash, hidden) {
    ...
    window.scrollTo(0, 0);
    var prevOv = root.style.overflow; root.style.overflow = 'visible';
    if (container !== root) container.style.overflow = 'visible';
    /* Y 측정: scrollTo(0,0) 후 getBoundingClientRect */
    var cTop = container.getBoundingClientRect().top + window.scrollY;
    var cW   = container.offsetWidth;
    var cH   = container.scrollHeight;
    function getY(el) { return Math.max(0, Math.round(el.getBoundingClientRect().top + window.scrollY - cTop)); }
    for (var i = 0; i < groups.length; i++) {
        groups[i].startY = (i === 0) ? 0 : getY(groups[i].el);
        groups[i].endY   = (i < groups.length - 1) ? getY(groups[i + 1].el) : cH;
    }
    var captures = [];
    var baseName = sajuxBuildCaptureBaseName(window.globalSajuData || null);
    var mime = sajuxCaptureImageMime();
    ...
    function next() {
        if (idx >= total) {
            ...
            if (!captures.length) { alert('캡처된 이미지가 없습니다. 새로고침 후 다시 시도해 주세요.'); sajuxHideCaptureOverlay(); cleanup(); return; }
            sajuxOfferCaptureDownload(captures, baseName, cleanup).catch(function(e) { console.warn('[sajux] zip err', e); cleanup(); });
            return;
        }
        var g = groups[idx];
        var h = Math.max(g.endY - g.startY, 1);
        sajuxUpdateCaptureProgress(idx, total, g.title);
        var fileBase = sajuxPadCaptureIndex(idx + 1) + '-' + sajuxSlugCaptureLabel(g.title);
        var tout = 30000;
        var done = false;
        var timer = setTimeout(function() { if (!done) { done = true; idx++; setTimeout(next, 16); } }, tout * 2);
        sajuxHtml2canvasRegion(container, 1, { w: cW, h: h, y: g.startY }, tout).then(function(canvas) {
            ...
            return sajuxCanvasToBlob(canvas, mime).then(function(blob) {
                ...
                idx++; setTimeout(next, 16);
            });
        }).catch(function(err) {
            if (done) return; done = true; clearTimeout(timer);
            console.warn('[sajux] mobile crop skip', idx + 1, err && err.message);
            idx++; setTimeout(next, 16);
        });
    }
    sajuxEnsureCaptureFonts().then(function() { return sajuxWaitImagesInRoot(container, 800); }).then(function() { setTimeout(next, 16); });
}
/* ───────────────────────────────────────────────────────────────── */
...
function generateDeepReport(data) {
    ...
    //   ⑤ 앞으로 올 월운 — 다음 달부터 12개월, 전반·후반 각 6개월 (사주 저장 2장)
...
function buildOpeningLifeNarrative(data) {
    ...
    else if (curAge) currentPhase = voiceTwilightPossessive(nm + 'curPhase') + ' 정리와 무엇을 남길지를 그려 가는 시기';
...
var ILJU_60_DB = {
    ...
    '丁未': {core:'촛불이 여름 대지 위에서 풍요롭게 타오르는 상(象). 일지 미(未)에는 정인·식신·비견 기운이 함께 깔려, 배움과 귀인으로 바닥을 깔아 두고 손끝 재능으로 풍요를 드러내는 결입니다. 겉은 부드럽고 감성적이나, 속에는 완성도와 표현 욕구가 함께 얽혀 있습니다.', strength:'감성적 풍요·표현력·예술적 재능·학습 본능', weakness:'준비만 길어지거나, 보여 주기만 하고 속이 비는 양극단을 번갈일 수 있음', love:'감성적이고 풍요로운 관계', career:'예술·음식·뷰티·엔터테인먼트·교육·기획', image:'여름 대지 위의 촛불 — 따뜻하고 풍요로운'},
...
function buildChapter1_Basic(data) {
    ...
    // 한 줄기 서사(buildPersonalPortrait)가 일주·인생 흐름 통변을 대체합니다 — 여기는 격국·조후·공망·직업·관계 보강만 둡니다.
    let _careerText = '', _loveText = '', _balanceText = '';

    // 조후(태어난 계절 기운)와 격국(사주 구조) 보강
    (function(){
...
${nmEunNeun(name)} ${seasonKr}에 태어났습니다. 이 계절의 기운이 사주 전체의 온도와 습도를 결정합니다. ${johu.split('.').slice(-3).join('.').trim()} 이 균형이 맞는 대운·세운이 찾아올 때 삶이 가장 크게 도약합니다.`;
        ...
        if(johuNarr || ggNarr) {
            _balanceText = (ggNarr + johuNarr).trim();
        }
    ...
    var balanceBlock = '';
    if (_balanceText) {
        balanceBlock = '<div class="sajux-glass-heavy sec-wonguk-balance" style="background:rgba(255,255,255,0.06);border:1px solid rgba(199,167,106,0.22);border-radius:12px;padding:20px 22px;margin:22px 0 0;backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);">'
            + '<div style="font-size:11px;color:var(--gold);margin-bottom:12px;letter-spacing:0.14em;font-weight:700;">격국 · 계절 균형</div>'
            + '<p style="font-size:13.5px;color:#ddd;line-height:2;margin:0;">' + boldStarsToStrong(voicePolishParagraph(data, _balanceText)) + '</p>'
            + '</div>';
    }

    var careerLoveBlock = '';
    if (_careerText || _loveText) {
        careerLoveBlock = '<div class="ilju-career-love-block sajux-glass-heavy" style="margin-top:22px;border:1px solid rgba(255,255,255,0.08);border-radius:12px;padding:22px;background:rgba(255,255,255,0.05);backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);">'
            + '<p style="font-size:12.5px;color:var(--text-dim);margin:0 0 18px;line-height:1.85;font-style:italic;">한 줄기 서사에 이어, 같은 원국을 직업·관계 언어로만 한 번 더 풀어 드립니다.</p>'
            + (_careerText ? '<div style="margin-bottom:22px;"><div style="font-size:11px;color:var(--gold);font-weight:700;letter-spacing:0.10em;margin-bottom:10px;">직업 방향 — 타고난 에너지가 빛나는 무대</div>'
                + '<p style="font-size:13.5px;color:#ddd;line-height:2.0;margin:0;">' + boldStarsToStrong(voicePolishParagraph(data, _careerText)) + '</p></div>' : '')
            + (_loveText ? '<div><div style="font-size:11px;color:var(--gold);font-weight:700;letter-spacing:0.10em;margin-bottom:10px;">관계 패턴 — 마음이 열리는 방식</div>'
                + '<p style="font-size:13.5px;color:#ddd;line-height:2.0;margin:0;">' + boldStarsToStrong(voicePolishParagraph(data, _loveText)) + '</p></div>' : '')
            + '</div>';
    }

    var supplementPieces = [];
    if (balanceBlock) supplementPieces.push(balanceBlock);
    if (gongmangText) supplementPieces.push(gongmangText);
    if (careerLoveBlock) supplementPieces.push(careerLoveBlock);
    if (!supplementPieces.length) return '';

    return '<div id="sec-wonguk-supplement" class="report-chapter chapter-start">'
        + supplementPieces.join('')
        + '</div>';
}
/** 십성 장·일주 보강용 — 업무형 한 줄(비견~정인) */
function getChapter3SipPersonalityLine(sip) {
    var sipDB = {
...
function getChapter3SipShortDb(sip) {
    return (window.SAJU_DB && window.SAJU_DB.SIPSEONG && window.SAJU_DB.SIPSEONG[sip]) || '주도적으로 판을 짜는 기질입니다.';
}

/** 원국 개별 십성 중 앞장서는 이름 최대 3개 — 상위 둘은 항상 풀이 대상으로 삼음 */
function getChapter3PrimarySipseongs(sipseong) {
    var entries = Object.keys(sipseong || {}).map(function (k) {
        return [k, Number(sipseong[k]) || 0];
    }).filter(function (x) {
        return x[1] > 0;
    }).sort(function (a, b) {
        return b[1] - a[1];
    ...
    var out = [entries[0][0]];
    if (entries.length > 1) out.push(entries[1][0]);
    if (entries.length > 2) {
        var c1 = entries[0][1], c2 = entries[1][1], c3 = entries[2][1];
        if (c3 >= c2 || (c1 >= 2 && c3 >= c1 - 1)) out.push(entries[2][0]);
    }
    return out.slice(0, 3);
}

function buildChapter3SipseongSynthesisParagraph(primaryList, data) {
    var list = primaryList || [];
    var nm = nmUi(data.name || '고객');
    ...
    var hasGwan = set['정관'] || set['편관'];
    var hasJae = set['정재'] || set['편재'];
 ...
 *  "○○님은 이런 사람입니다" — 1부에서 일주 카드 바로 다음에 두어 한 줄기 서사가 먼저 이어지게 한다.
...
/** ── 앞으로의 운 · 운세 카드 문장 생성 법칙 (템플릿·LLM 공통) ── */
var SAJUX_FORTUNE_WRITE = {
    layers: {
        daeun: '10년 큰 기후만. 해마다·달마다 풀지 말 것.',
        seyun: '그해 분위기 + 지금 10년과의 차이 1줄 + 행동 1줄. 대운 장문·월운 반복 금지.',
        wolun: '그달만. 「○월은 ○○쪽이 앞에」 고정 서두 금지. 행동 1줄은 월마다 다른 문장.'
    },
    rules: [
        '고객 본문: 십성 한자명·(십성) 괄호 노출 금지 → SAJUX_SIP_MODERN 현대어 키워드만.',
        '차트·간지 표기(경자 등)는 유지. 명리 교과서체(용신(用)·생극 장황 설명)는 1~2문장으로 압축.',
    ...
    action: {
        daeun: {
            good: [
                '미뤄 두신 큰 일 — 결혼·이직·창업·자격 — 가운데 하나만 이 10년 안에 매듭지으십시오.',
                '평생 끌고 갈 축 한두 가지에만 힘을 모으면, 이 10년이 가장 단단한 기둥이 됩니다.',
                '분기마다 “이번 분기에 끝낼 한 가지”만 적어 두고 밀어 보십시오.'
            ],
            mild: [
                '한 해에 한 번, 쌓인 것을 한 줄로 정리해 두시면 10년 뒤 자산이 됩니다.',
                '검증된 일 한 가지에만 속도를 올리고, 나머지는 평소 페이스를 유지하십시오.',
                '끝맺음을 책임질 동료 한 명만 옆에 두십시오.'
            ],
            flat: [
                '끊을 것·이어 갈 것을 한 가지씩만 가르는 데 이 10년을 쓰시면 다음이 가벼워집니다.',
                '새로 벌이기보다 손에 쥔 일의 깊이를 한 단계만 올리십시오.',
                '거래처·가까운 다섯 명과의 약속을 문장으로 남기십시오.'
            ],
            caution: [
                '신규 보증·동업·대출은 이 구간에서 가급적 늘리지 마십시오.',
                '큰 결정은 글로 받은 뒤, 하룻밤 넘기고 다시 보십시오.',
                '건강·가족·핵심 거래처만 흔들리지 않게 챙기는 쪽이 이득입니다.'
            ],
            tough: [
                '이 10년은 “지키는 한 가지 목표”만 두고, 새로 벌이는 폭을 줄이십시오.',
                '무사히 지나신 분이 다음 10년에서 가장 단단히 서는 경우가 많습니다.',
                '중요한 약속은 구두 말고 문장으로만 남기십시오.'
            ]
        },
        seyun: {
            good: [
                '올해는 미뤄 둔 안건 한 가지를 매듭지으시면 다음 해가 가벼워집니다.',
                '분기마다 큰 결정은 한 가지만 잡으십시오.',
                '발표·계약·면접 중 하나를 올해 일정의 중심에 두십시오.',
                '올해 만든 결과물 한 가지를 밖으로 드러내 보십시오.'
            ],
            mild: [
                '지난해까지 쌓은 것이 작은 결실로 돌아오는 해예요.',
                '한 해의 성과를 분기마다 한 줄씩 기록해 두십시오.',
                '검증된 루틴 하나만 유지해도 충분합니다.'
            ],
            flat: [
                '새 시작보다 “끊을 한 가지·이어 갈 한 가지”를 가르는 해예요.',
                '숫자·역할·약속을 한 장으로 정리하는 데 시간을 쓰십시오.',
                '큰 변화보다 마무리 한 건에 집중하십시오.'
            ],
            caution: [
                '큰돈·큰 관계·큰 자리를 동시에 흔들지 마십시오.',
                '새 계약은 한 분기 미루고 다시 보십시오.',
                '이번 해는 속도보다 검토 한 번이 이깁니다.'
            ],
            tough: [
                '새 투자·창업·이직은 내년 이후로 미루시고, 지키는 쪽에만 무게를 두십시오.',
                '지금 있는 것을 점검하는 데만 힘을 쓰셔도 출발선이 됩니다.',
                '건강 검진·수면 루틴을 올해 안에 한 번 고정하십시오.'
            ]
        },
        wolun: {
            good: [
                '이달 안에 미뤄 둔 안건 한 가지를 끝내 보십시오.',
                '계약·발표·중요한 대화 중 하나를 이번 달 중순에 잡으십시오.',
                '완성본 한 장을 먼저 만들고 설명하십시오.'
            ],
            mild: [
                '작은 시작 하나와 작은 마무리 하나만 잡으십시오.',
                '지난달 리듬을 유지하되, 연락 한 통만 더하십시오.',
                '주 1회, 20분만 “다음 달 할 일”을 적어 두십시오.'
            ],
            flat: [
                '새 일보다 수면·식사 시간을 먼저 맞추십시오.',
                '구독·고정비·미수금 표만 열어 보는 주를 한 번 두십시오.',
                '관계에서 미뤄 둔 답장 한 통을 보내 보십시오.'
            ],
            caution: [
                '이달 큰 결정은 한 가지만, 나머지는 다음 달로 미루십시오.',
                '서두르지 말고 일정을 반으로 나누어 잡으십시오.',
                '감정이 올랐을 때는 하룻밤 넘기고 말하십시오.'
            ],
            tough: [
                '큰돈·큰 약속·큰 변화는 멈추고 점검만 하십시오.',
                '중요한 서류·계약은 이번 달 안에만 검토하십시오.',
                '몸이 보내는 신호(수면·소화)를 먼저 챙기십시오.'
            ]
        }
    }
};

function getSajuxFortuneNarrativeRulesBlock() {
    var w = SAJUX_FORTUNE_WRITE;
    return [
        '[운세 카드 문장 생성 법칙 — 대운·세운·월운]',
        w.layers.daeun,
        w.layers.seyun,
        w.layers.wolun,
        ''
    ].concat(w.rules).concat([
        '',
        '십성 현대어: ' + (typeof getSajuxSipseongModernPromptBlock === 'function' ? 'getSajuxSipseongModernPromptBlock() 참고' : 'SAJUX_SIP_MODERN')
    ]).join('\n');
}

function fortunePhraseLedger() {
    return { lines: [], snippets: [] };
}

function fortuneSnippetUsed(ledger, snippet) {
    var s = String(snippet || '').slice(0, 48);
    if (!s || ledger.snippets.indexOf(s) >= 0) return true;
    ledger.snippets.push(s);
    return false;
}

function pickFortuneActionUnique(kind, tone, seed, ledger) {
    var pool = (SAJUX_FORTUNE_WRITE.action[kind] && SAJUX_FORTUNE_WRITE.action[kind][tone])
        || (SAJUX_FORTUNE_WRITE.action[kind] && SAJUX_FORTUNE_WRITE.action[kind].flat)
        || [];
    if (!pool.length) return '';
    var start = Math.abs(hashSeed(seed)) % pool.length;
    var i;
    for (i = 0; i < pool.length; i++) {
        var line = pool[(start + i) % pool.length];
        if (ledger.lines.indexOf(line) < 0) {
            ledger.lines.push(line);
            return line;
        }
    }
    var fallback = pool[start];
    ledger.lines.push(fallback);
    return fallback;
}

/** 운세 카드 본문 — 십성 현대어 + 괄호 조사·이중 종결 정리 (HTML 전 narrativeVp) */
function fortunePolishPlain(data, text) {
    var s = String(text == null ? '' : text);
    s = voiceModernizeSipseong(s);
    s = fixKoreanNarrativeGlitch(s);
    return s;
...
function fortuneSipModernPhrase(data, sip, role) {
    var kw = fortuneSipModernKw(sip);
    if (!kw) return '';
    var gk = sipToSipGroupKey(sip);
    var scene = gk ? (SAJUX_SIP_WEAPON_SCENE[gk] || null) : null;
    if (role === 'gan') {
        return '일·관계에서는 **' + kw + '**' + getJosaFlex(kw, '이/가') + ' 먼저 움직이기 쉽습니다.';
    }
    if (role === 'ji') {
        var ex = scene && scene.example ? scene.example.split('거나')[0] : '현장·마무리';
        return '돈·몸·현장에서는 **' + kw + '**으로 ' + ex + ' 쪽이 편합니다.';
    }
    return '**' + kw + '**' + getJosaFlex(kw, '이/가') + ' 이 시기의 중심에 가깝습니다.';
}

/** 고객-facing 시기 풀이 — 십성명 노출 없음, 2~3문장 */
function sajuxNarratePeriodCustomer(data, g, j, seed) {
    if (!g || !j) return '';
    var a = sajuxAnalyzePeriod(data, g, j);
    var nm = data.name || '고객';
    var parts = [];
    parts.push(pickVoiceLine([
        '<strong>' + a.gzKr + '</strong> 시기에는 겉으로 드러나는 방향과 속바탕이 함께 움직입니다.',
        '<strong>' + a.gzKr + '</strong> — 한 시기의 결이 이렇게 잡혀 있습니다.'
    ], seed + '|open'));
    if (a.sipGan) parts.push(fortuneSipModernPhrase(data, a.sipGan, 'gan'));
    if (a.sipJi && a.sipJi !== a.sipGan) parts.push(fortuneSipModernPhrase(data, a.sipJi, 'ji'));
    if (a.yongHit) {
        parts.push(pickVoiceLine([
            nmUi(nm) + '에게 잘 맞는 기운이 이 시기에 들어와, 평소보다 체감이 큽니다.',
            '원국에서 비어 있던 쪽을 이 시기에 채우는 흐름에 가깝습니다.'
        ], seed + '|yong'));
    } else if (a.heeHit) {
        parts.push('무리하지 않아도 흐름이 붙기 쉬운 시기예요.');
    } else if (a.giHit) {
        parts.push('크게 벌리기보다 지키고 정리하는 쪽이 ' + nmUi(nm) + '에게 맞습니다.');
    } else if (a.fillsWeak) {
        parts.push('평소 약했던 ' + (SAJUX_OH_KR_SHORT[a.natal.weakest] || '') + ' 기운이 보충되는 시기입니다.');
    }
    return parts.join(' ');
}

function upcomingSeyunYearHook(g, j, seed) {
    var hooks = {
        '甲': ['새 과목·새 프로젝트가 들어오기 쉬운 해예요.', '배움과 확장이 앞서는 해입니다.'],
        '乙': ['사람·협업으로 길이 열리는 해예요.', '관계가 일의 속도를 결정합니다.'],
        '丙': ['만든 것·말한 것이 밖으로 드러나는 해예요.', '표현과 인정이 앞서는 해입니다.'],
        '丁': ['한 분야를 깊게 파면 바로 보상이 따라오는 해예요.', '집중이 곧 평가로 이어집니다.'],
        '戊': ['천천히 쌓이는 해예요.', '신뢰와 안정이 앞서는 해입니다.'],
        '己': ['정리·마무리·연장이 앞서는 해예요.', '결산과 다듬기에 좋습니다.'],
        '庚': ['잘라 내고 기준을 세우면 이득인 해예요.', '결단이 승부를 가릅니다.'],
        '辛': ['완성·브랜딩이 빛나는 해예요.', '다듬은 것이 인정받기 쉽습니다.'],
        '壬': ['정보·인맥·통로가 넓어지는 해예요.', '연결이 기회가 됩니다.'],
        '癸': ['공부·연구·내공을 채우기 좋은 해예요.', '조용히 쌓는 해입니다.']
    };
    var ji = {
        '子': '기획·문서·집중이 빛납니다.',
        '丑': '느리지만 단단히 쌓입니다.',
        '寅': '활동·이동이 따라옵니다.',
        '卯': '인맥·협업이 열립니다.',
        '辰': '변수가 있어 큰 결정은 분기 초·말에만 잡으세요.',
        '巳': '미루던 결정이 풀리기 쉽습니다.',
        '午': '성과·인정이 한 번 따라옵니다.',
        '未': '창작·교육 쪽으로 길이 열립니다.',
        '申': '판단 속도가 승부를 가릅니다.',
        '酉': '그동안 쌓은 노력이 결실로 돌아옵니다.',
        '戌': '끊을 것과 남길 것을 가르기 좋습니다.',
        '亥': '큰 무대보다 내실을 채우는 해입니다.'
    };
    var a = pickVoiceLine(hooks[g] || ['이 해만의 결이 있습니다.'], seed + '|g');
    var b = ji[j] || '';
    return b ? (a + ' ' + b) : a;
}

function upcomingSeyunCompareLine(data, yr, g, j, tone, seed, ledger) {
    var daeunCtx = sajuxGetDaeunContext(data, yr);
    if (!daeunCtx || typeof sajuxFortuneScore !== 'function') return '';
    ...
    var delta = yrSc - dSc;
    if (delta >= 2) {
        var up = pickVoiceLine([
            '지금 겪는 10년보다 이 해가 한 단계 밝습니다.',
            '큰 10년 흐름 위에서 올해가 눈에 띄는 해가 될 수 있어요.'
        ], seed + '|up');
        return fortuneSnippetUsed(ledger, up) ? '' : up;
    }
    if (delta <= -2 && (tone === 'caution' || tone === 'tough' || tone === 'flat')) {
        var dn = pickVoiceLine([
            '큰 10년 흐름보다 숨 고르는 해로 잡으시면 낫습니다.',
            '10년 바탕은 무겁고 올해는 정리·점검에 맞추는 편이 좋아요.'
        ], seed + '|dn');
        return fortuneSnippetUsed(ledger, dn) ? '' : dn;
    }
    return '';
}
...
(function bindFortuneNarrativePrompt() {
    try { SAJUX_VOICE.fortuneNarrativePrompt = getSajuxFortuneNarrativeRulesBlock(); } catch (e) { /* early parse */ }
})();
...
function sajuxNarratePeriod(data, g, j, seed) {
    ...
    if (a.sipGan) {
        var kwG = fortuneSipModernKw(a.sipGan);
        if (kwG) parts.push('천간은 **' + kwG + '**' + getJosaFlex(kwG, '이/가') + ' ' + nmEulReul(nm) + ' 둘러싸며, 이 시기 일·관계의 결이 여기서 갈립니다.');
    }
    if (a.sipJi && a.sipJi !== a.sipGan) {
        var kwJ = fortuneSipModernKw(a.sipJi);
        if (kwJ) parts.push('지지는 **' + kwJ + '**' + getJosaFlex(kwJ, '으로/로') + ' 현장·몸·돈의 결을 만듭니다.');
    }
    return parts.join(' ');
}
...
function stripLifeArcJargon(html) {
    ...
    s = s.replace(/그중\s+\d+세[^\s]*\s*(부터|쯤|무렵)[^.]*\.\s*/g, '');
...
function buildCurrentPeriodCard(data) {
    ...
    // ── 짧은 한눈 헤더 (대운 전폭 · 세운·월운 하단 2열) ──
    function chipBox(label, color, ko, hj, widthCss) {
        return '<div style="background:rgba(255,255,255,0.04);border-radius:10px;padding:10px 14px;border-left:3px solid ' + color + ';' + widthCss + 'box-sizing:border-box;">'
    ...
    var snapshotHtml = '<div style="display:flex;flex-wrap:wrap;gap:10px;margin:14px 0 24px;">'
        + chipBox('현재 대운', toneColor(dTone), dGanKr + dJiKr, dGan + dJi, 'flex:0 0 100%;width:100%;')
        + chipBox('현재 세운', toneColor(yTone), yGanKr + yJiKr, yGan + yJi, 'flex:1 1 calc(50% - 5px);min-width:120px;')
        + chipBox('현재 월운', toneColor(mTone), mGanKr + mJiKr, mGan + mJi, 'flex:1 1 calc(50% - 5px);min-width:120px;')
        + '</div>';
    // ── 1) 인트로 ──
    ...
    var closing = '이렇게 ' + nmUi(name) + ' 지금 시기를 — 가장 큰 대운부터 한 달의 월운까지, 그리고 그 위에 비친 다섯 자리의 모양까지 — 한 흐름으로 풀어 드렸어요. 다음 챕터에서는 이 흐름이 “앞으로” 어떻게 이어지는지 — 다가올 대운 두 개와, 다음 해부터의 10년, 다음 12개월 월운(전반·후반 각 6개월)까지 차근차근 짚어 드릴게요.';
...
function buildUpcomingDaewunCards(data) {
    ...
    var fortuneLedger = fortunePhraseLedger();
    var cards = nextRows.map(function (nx, idx) {
        ...
        var body = buildUpcomingDaeunBody(data, g, j, nx.age, t, fortuneLedger);
        var col = toneColor(t);
...
function buildUpcomingSewunCards(data) {
    ...
    var seyunLedger = fortunePhraseLedger();
    var cards = '';
    for (var k = 0; k < 10; k++) {
        var yr = startYear + k;
        var gI = ((yr - 4) % 10 + 10) % 10;
        ...
        var t = tone(sc);
        var col = toneColor(t);
        var body = upcomingSeyunBodyBrief(data, yr, g, j, t, seyunLedger);
...
/** ─────────────────────────────────────────
 *  buildUpcomingWolunMonthCardsHtml — 월운 카드 N개월 (사주 저장 6+6 분할용)
 * ───────────────────────────────────────── */
function buildUpcomingWolunMonthCardsHtml(data, opts) {
    opts = opts || {};
    var count = opts.count || 6;
    var skip = opts.skip || 0;
    if (!data || !data.dayStem || count < 1) return { html: '', rangeLabel: { from: '', to: '' } };

    var baseNow = (typeof getReportBaseDate === 'function') ? getReportBaseDate(data) : new Date();
    var startYear = baseNow.getFullYear();
    var startMonth = baseNow.getMonth() + 1;
    ...
    var wolunLedger = fortunePhraseLedger();
    var cards = '';
    ...
    while (added < count) {
        ...
        var t = tone(sc);
        var col = toneColor(t);
        var body = upcomingWolunBodyBrief(data, yrPointer, moPointer, mGan, mJi, t, wolunLedger);
        var ym = yrPointer + '년 ' + moPointer + '월';
        if (!rangeLabel.from) rangeLabel.from = ym;
        rangeLabel.to = ym;
        cards += '<div class="sajux-wolun-month-card" style="background:rgba(255,255,255,0.03);border-radius:10px;padding:14px 16px;border-left:3px solid ' + col + ';margin-bottom:10px;">'
              + '<div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;margin-bottom:6px;">'
              + '<div style="font-size:14.5px;font-weight:800;color:#fff;font-family:\'Noto Sans KR\',sans-serif;">' + ym + ' <span style="color:var(--gold);margin-left:4px;">' + (HK[mGan] || mGan) + (HK[mJi] || mJi) + '</span><span style="font-size:11px;color:#888;margin-left:4px;">(' + mGan + mJi + ')</span></div>'
              + '<span style="font-size:10px;background:rgba(255,255,255,0.06);color:' + col + ';padding:2px 8px;border-radius:8px;letter-spacing:0.03em;">' + toneLabel(t) + '</span>'
              + '</div>'
    ...
    return { html: cards, rangeLabel: rangeLabel };
}

/** 다음 달부터 12개월 월운 — 사주 저장 시 전반·후반 각 6개월(이미지 2장) */
function buildUpcomingWolunCards(data) {
    if (!data || !data.dayStem) return '';

    var half1 = buildUpcomingWolunMonthCardsHtml(data, { count: 6, skip: 0 });
    var half2 = buildUpcomingWolunMonthCardsHtml(data, { count: 6, skip: 6 });
    var subA = (half1.rangeLabel.from && half1.rangeLabel.to)
        ? (half1.rangeLabel.from + ' ~ ' + half1.rangeLabel.to)
        : '다음 달부터 6개월';
    var subB = (half2.rangeLabel.from && half2.rangeLabel.to)
        ? (half2.rangeLabel.from + ' ~ ' + half2.rangeLabel.to)
        : '이어지는 6개월';

    var chHeadA = buildSectionHeader('upcomingWolunA', data, { headerMode: 'mainSub', subTitle: subA, skipIntro: true });
    var chHeadB = buildSectionHeader('upcomingWolunB', data, { headerMode: 'mainSub', subTitle: subB, skipIntro: true });

    return '<div class="report-chapter" id="sec-upcoming-wolun">'
        + '<div class="sajux-wolun-capture-half">'
        + chHeadA
        + buildNarrativePara(data, '아래는 <strong>그달의 리듬과 행동 한 가지</strong>만 담았어요. 세운·대운 설명은 생략했습니다. 밀고 싶은 달·쉬고 싶은 달만 골라 보셔도 됩니다.', { lineHeight: '2', marginBottom: '14px' })
        + '<div style="display:flex;flex-direction:column;gap:0;">' + half1.html + '</div>'
        + '</div>'
        + '<div class="sajux-wolun-capture-half" style="margin-top:28px;">'
        + chHeadB
        + buildNarrativePara(data, '전반 6개월에 이어지는 흐름입니다. 후반도 <strong>한 달씩</strong> 같은 방식으로 읽어 주시면 됩니다.', { lineHeight: '2', marginBottom: '14px' })
        + '<div style="display:flex;flex-direction:column;gap:0;">' + half2.html + '</div>'
        + '</div>'
        + '</div>';
}
...
function buildChapter3_Sipseong(data) {
    ...
    function distributePercentInts(sums, target) {
        var t = sums.reduce(function (a, b) { return a + b; }, 0) || 1;
        var floats = sums.map(function (s) { return (target * s) / t; });
        var out = floats.map(function (f) { return Math.floor(f + 1e-9); });
        var rem = target - out.reduce(function (a, b) { return a + b; }, 0);
        var order = floats.map(function (f, i) {
            return { i: i, r: f - Math.floor(f + 1e-9) };
        }).sort(function (a, b) { return b.r - a.r; });
        for (var z = 0; z < rem; z++) {
            if (order[z]) out[order[z].i]++;
        }
        return out;
    }
    var normInts = distributePercentInts(groupSums, 100);
    const groupRank = SIP_BAR_GROUPS.map(function (g, idx) {
        return {
            key: g.key,
            label: g.label,
...
function buildChapter_HapGyeok(data) {
    ...
    const hireLine = {
        ...
        thin: '관성(官)이 약한 편이라, 큰 조직보다는 **중소·전문·소규모 팀**에서 ' + nmUi(name) + ' 결이 더 잘 살아납니다. 큰 회사에 들어가신다면 그 안에서도 “전문가 트랙”을 일찍 잡으시는 게 좋습니다.',
    ...
    function box(label, val, b, body) {
        return '<div class="hapgyeok-axis-card" style="background:rgba(255,255,255,0.04);border-radius:12px;padding:18px 20px;width:100%;box-sizing:border-box;">'
            + '<div style="display:flex;flex-wrap:wrap;justify-content:space-between;align-items:center;gap:10px 16px;margin-bottom:12px;">'
            + '<div style="font-size:12.5px;letter-spacing:0.04em;color:rgba(255,255,255,0.72);font-weight:600;min-width:0;flex:1 1 auto;">' + label + '</div>'
            + '<div style="display:flex;align-items:center;gap:10px;flex-shrink:0;">'
            + '<span style="font-size:15px;font-weight:700;color:' + b.col + ';">' + val + '%</span>'
            + '<span style="font-size:10.5px;background:rgba(255,255,255,0.08);color:' + b.col + ';padding:3px 9px;border-radius:6px;letter-spacing:0.04em;white-space:nowrap;">' + b.label + '</span>'
            + '</div></div>'
            + '<p style="font-size:13.5px;color:#ccc;line-height:1.92;margin:0;max-width:100%;">' + boldStarsToStrong(_vp(body)) + '</p>'
            + '</div>';
    }
    ...
    return '<div style="display:flex;flex-direction:column;align-items:center;gap:5px;padding:2px 0;">'
        + stems.map(function(ch) {
            ...
            var hanStyle = 'font-size:14px;font-weight:300;font-family:\'Noto Sans KR\',\'Noto Sans SC\',sans-serif;'
                + (cls === 'water' ? 'color:var(--water,#1a6fd4);' : '');
...
/** 만세력 표 하단 — 수(水) 색상 표기 안내 */
function buildManseWaterColorNoteHtml() {
    return '<p class="manse-water-color-note" style="margin:8px 0 0;font-size:9.5px;color:var(--text-dim,rgba(255,255,255,0.52));line-height:1.58;letter-spacing:0.01em;text-align:left;">'
        + '※ 명리 전통에서는 <strong style="color:var(--text-soft,rgba(232,228,218,0.88));font-weight:600;">수(水)</strong>를 흑색(玄色)으로 씁니다. '
        + '어두운 화면에서 임·계·子·亥가 잘 보이도록, 본 표의 <strong style="color:var(--water,#1a6fd4);font-weight:600;">수 오행 한자·오행 막대만 파란색</strong>으로 표기했습니다.'
        + '</p>';
}
...
function buildVipEvidenceBlock(data) {
    ...
    if(pillars.length>=2){
        ...
        pillars.forEach(function(p,i){
            ...
            var gKr=HK_GAN[g]||g;
            var hanjaStyle='font-size:1.55em;font-weight:300;color:'+(STEM_OH[g]==='water'?'var(--water,#1a6fd4)':col)+';font-family:\'Noto Sans KR\',\'Noto Sans SC\',sans-serif;';
        ...
        pillars.forEach(function(p,i){
            ...
            var jKr=HK_JI[j]||j;
            var hanjaStyle='font-size:1.55em;font-weight:300;color:'+(BRNCH_OH[j]==='water'?'var(--water,#1a6fd4)':col)+';font-family:\'Noto Sans KR\',\'Noto Sans SC\',sans-serif;';
        ...
        ev += '</tr>';
        ev += '</table></div>';
        ev += buildManseWaterColorNoteHtml();
    }
    ev += '</div>';
...
function buildChapter9_Remedy(data) {
    ...
    var yongBg = ({wood:'rgba(79,195,161,0.06)',fire:'rgba(255,112,67,0.06)',earth:'rgba(255,202,40,0.06)',metal:'rgba(176,190,197,0.06)',water:'rgba(100,181,246,0.06)'})[yong] || 'rgba(255,255,255,0.04)';
    var yongRoutineText = ({
    ...
    return buildChapterWithSingleIntro('remedy', data,
        ...
        + '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">'
        + '<div style="padding:14px 16px;border-radius:10px;background:rgba(255,210,150,0.05);border-left:2px solid rgba(255,210,150,0.45);"><div style="font-size:11px;color:#ffd9a8;font-weight:700;letter-spacing:0.05em;margin-bottom:6px;">🌅 아침 (6~9시)</div><p style="font-size:13px;color:#e8e0d2;line-height:1.85;margin:0;">' + dailyRx.morn + '</p></div>'
...
function buildLifePanoramaSection(data) {
    ...
    var closeSceneByHour = {
        ...
        '酉': '날이 접히며 소리가 맑아지는 시간대의 기운은, 완성된 결과물과 깔끔한 인사로 무대를 닫으려 합니다.',
    ...
    if (late.length) {
        ...
        lateMood = pickVoiceLine([
            '말년 무렵의 호흡은 길어 보일수록 한 박자씩 천천히 줄어드는 편으로 읽힙니다.',
            '여생의 숨결은 조용히 정리하고 남기고 싶은 것만 남기려는 마음과 겹쳐 그려질 가능성이 큽니다.'
        ], String(name || '') + 'lateMoodDef');
    }
    ...
    if (data.birthTimeKnown === false) {
        closeScene = '태어난 시각을 특정할 수 없어 시간대별 상징은 열어 두고, ' + voiceTwilightPossessive(name + 'closeU') + ' 온도는 삼주(여섯 글자)가 쌓아 온 패턴으로 읽는 편이 맞습니다.';
    }
    var p4 = '마지막 장면. ' + lateMood + ' ' + closeScene + ' '
        + '눈을 감으실 때도 — 평생 몸에 배어 온 호흡과 태도 그대로, 소란 없이 손을 내려놓으려는 결로 읽히는 편입니다. '
        + '언제 어떻게인지를 적어 두는 예언이 아니라, 마무리를 마음으로 한 번 짚어 보는 말입니다. '
    ...
    var timeFoot = (data.birthTimeKnown === false)
        ? '<p style="font-size:11px;color:rgba(180,186,198,0.72);line-height:1.75;margin:14px 0 0;">*(태어난 시간을 알 수 없어, 삼주(여섯 글자)의 누적된 기운을 바탕으로 ' + voiceTwilightPossessive(name + 'tfNote') + ' 방향성을 도출했습니다.)</p>'
        : '';
    ...
    var c5_p2 = ' 70대 이후에 ' + nmDnim(name) + '이 남기실 것은 — ' + (legacy || '평생 다듬어 온 ' + nmUi(name) + ' 결')
        ...
        + ' 마지막 순간의 풍경은 — 시각대의 상징으로는 ' + (closeScene || '평생 일군 성벽과 그 안의 온기를 남기고 떠나시는 결입니다.')
...
function buildTOC(data) {
    data = data || {};
    var nmLine = (typeof nmDnim === 'function') ? nmDnim(data.name) : ((data.name || '고객') + '님');
    ...
    [0, 1, 2, 3, 4].forEach(function (p) {
        ...
        var countBadge = keys.length ? (' · ' + keys.length + '절') : '';
        body += '<div style="' + gHead + '">' + pm.head + countBadge + '<span style="' + gSub + '">' + pm.sub + '</span></div>';
        keys.forEach(function (k) {
            var r = SAJUX_SECTION_REGISTRY[k];
...
var HELP_DATA = {
    manse: {
        title: "만세력 원국 (사주팔자)",
        desc: "태어난 연, 월, 일, 시간을 기준으로 우주의 기운(간지)을 8개의 글자로 표현한 나의 고유한 바코드입니다.<br><br>• <b>일원(일주)</b>: 나 자신과 배우자를 의미합니다.<br>• <b>월주</b>: 나의 부모, 타고난 환경, 직업적 적성을 뜻합니다.<br>• <b>년주</b>: 조상, 초년기, 국가적 스케일을 나타냅니다.<br>• <b>시주</b>: 자식, 황혼 운, 은밀한 속마음을 나타냅니다."
    },
    relation: {
        title: "합 · 충 · 형 · 파 · 해",
...
var SHINSAL_LOGIC = {
    ...
    // ── 9. 망신살 — 연지·일지 기준 (亡神: 삼합별 申子辰亥·亥卯未寅·寅午戌巳·巳酉丑申) ──
    '망신살': (gz, ec) => {
        const yb = ec.getYear()[1]; const db = ec.getDay()[1];
...
function runAnalysis(overrideParams) {
    ...
    showLoading('만세력을 정밀 계산하는 중입니다', () => {
        ...
        document.getElementById('manse-table').innerHTML = manseHtml + buildManseWaterColorNoteHtml();
...
var strat = _dText(s>=2 ? STRAT_GOOD.join('<br>') : s>=0 ? STRAT_MID.join('<br>') : STRAT_BAD.join('<br>'));
        ...
        (function() {
            ...
            const eras = [
                ...
                {era:'🌙 황혼 (60세~)', filter: d=>(d.getStartAge()-1)>=60},
            ];
