...
function renderDashboard(data) {
    ...
    // 3. Wuxing & Sipseong Bars (복구: 십성 바 추가)
    let wHtml = '';
    ...
    sipKeys.forEach(k => {
        // Simple mapping from detail to category
        let val = 0;
    ...
    // 5. Daewun Table (복구: 가로 스크롤 카드)
    // calculation_engine에서 대운 리스트를 가져와야 함. 일단 간단한 목업으로 UI 복구 확인
    let dHtml = '<div class="fortune-scroll">';
    for(let i=0; i<8; i++) {
        const age = data.daewunNum + (i*10);
        dHtml += \`<div class="f-card">
            <div class="f-head">\${age}세</div>
            <div class="f-hz">? ?</div>
            <div class="f-sip">대운</div>
        </div>\`;
    }
    dHtml += '</div>';
